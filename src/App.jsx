...
export default function ERP(){
  ...
  const PgDRE=()=>{
    ...
    // Despesas: excluir entradas geradas automaticamente dos pedidos (comissões/materiais já em cc e cm)
    const despesasFin=financeiro.filter(f=>f.tipo==="pagar"&&!f.pedidoId).reduce((s,f)=>s+f.parcelas.filter(p=>p.venc?.startsWith(anoStr)).reduce((ss,p)=>ss+p.valor,0),0);
    const despesasFinPagas=financeiro.filter(f=>f.tipo==="pagar"&&!f.pedidoId).reduce((s,f)=>s+f.parcelas.filter(p=>p.venc?.startsWith(anoStr)&&p.pago).reduce((ss,p)=>ss+p.valor,0),0);
    ...
    const mensalData=MESES.map((m,i)=>{
      ...
      const pagM=financeiro.filter(f=>f.tipo==="pagar"&&!f.pedidoId).reduce((s,f)=>s+f.parcelas.filter(p=>p.venc?.startsWith(prefx)).reduce((ss,p)=>ss+p.valor,0),0);
      return{name:m,receita:recM,despesas:pagM,resultado:recM-pagM};
    });
    // Categorias de despesa para o ano
    const byCat={};financeiro.filter(f=>f.tipo==="pagar"&&!f.pedidoId).forEach(f=>{const pags=f.parcelas.filter(p=>p.venc?.startsWith(anoStr));if(!pags.length)return;const v=pags.reduce((s,p)=>s+p.valor,0);const c=f.categoria||"Outros";byCat[c]=(byCat[c]||0)+v;});
