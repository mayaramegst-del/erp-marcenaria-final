    ...
    try{
      ...
      while(pageStart<canvas.height){
        ...
        if(pageEnd<canvas.height){
          let safeEnd=pageEnd;
          // Recua o corte para antes do bloco, mas só se o desperdício for < 20% da página.
          // Blocos que começam cedo (ex: 44%) não são recuados — ficam cortados naturalmente.
          for(const rb of rowBounds){
            if(rb.topPx<safeEnd&&rb.botPx>safeEnd&&(safeEnd-rb.topPx)<pageHpx*0.2) safeEnd=rb.topPx;
          }
    ...
    return(
      <div className="ct-page">
        ...
        <div className="ct-clausula">
          ...
          <p className="ct-paragrafo">§ 4º: O prazo de execução ficará automaticamente suspenso nos períodos em que o CONTRATANTE não disponibilizar acesso ao local para a execução dos serviços ou deixar de efetuar o pagamento da parcela acordada, retomando sua contagem somente após a regularização, sem qualquer penalidade à CONTRATADA.</p>
        </div>
        ...
        <div className="ct-clausula">
          ...
          <div className="ct-clausula-corpo">{`O CONTRATANTE compromete-se a:\n\nI — Efetuar os pagamentos nas datas e condições acordadas neste contrato;\nII — Disponibilizar o local para medição, produção e instalação nas datas combinadas, sendo que a ausência de acesso por mais de 5 (cinco) dias corridos acarretará a suspensão automática do prazo de execução;\nIII — Comunicar à CONTRATADA qualquer vício ou problema no prazo máximo de 30 (trinta) dias após a instalação;\nIV — Não realizar modificações nos móveis sem prévia autorização da CONTRATADA durante o período de garantia;\nV — Manter os pagamentos em dia, sendo que o atraso superior a 30 (trinta) dias corridos facultará à CONTRATADA a suspensão dos serviços até a regularização, sem que isso configure inadimplência ou enseje qualquer penalidade à CONTRATADA.`}</div>
        </div>

        <div className="ct-clausula">
          <div className="ct-clausula-titulo">{cn()} — Da Rescisão</div>
          <div className="ct-clausula-corpo">{`O presente contrato poderá ser rescindido por qualquer das partes mediante notificação por escrito com antecedência mínima de 10 (dez) dias, observando-se:\n\nI — Rescisão pelo CONTRATANTE após o início da produção: devidos os valores proporcionais aos serviços já executados, acrescidos de multa de 10% (dez por cento) sobre o saldo remanescente do contrato;\nII — Rescisão imotivada pela CONTRATADA após o início da produção: sujeita ao pagamento de multa equivalente a 10% (dez por cento) sobre o saldo remanescente do contrato, além da devolução dos valores pagos proporcionais aos serviços não executados. Esta penalidade não se aplica quando a rescisão for motivada por inadimplência do CONTRATANTE, hipótese em que a CONTRATADA devolverá apenas os valores proporcionais aos serviços não executados, deduzidos os materiais já adquiridos nos termos do Item III;\nIII — Em qualquer hipótese de rescisão, os materiais já adquiridos e especificados exclusivamente para o projeto serão descontados do valor a restituir, mediante apresentação de comprovação.`}</div>
        </div>
...
export default function ERP(){
  ...
  const [dreCatDetalhe,setDreCatDetalhe]=useState(null);
  ...
  const importarOrcAnaPaula=()=>{
    ...
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("ana paula"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"Ana Paula e Silas Mariano",tel:"",email:"",endereco:"Alameda Leônidas de Almeida Leme, Lote 6, Quadra R - Residencial Monte Alegre, Piracicaba-SP",doc:""}]);}
    const ambs=[
      {id:uid(),nome:"Hall de Entrada – Painel Espelho",desc:"Painel em MDF Jequitibá (Arauco), 691×277cm. Espelho embutido com bordas: superior 40cm, inferior 40cm, laterais 55cm. Recorte 45° nas quinas. Divisão em 3 partes iguais com friso de 1mm entre elas. 1 porta de giro (para dentro) mimetizada com frisos de 1cm no painel.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha – Armários Superiores",desc:"Armários aéreos em MDF Areia (Guararapes) ou Off White (Duratex), largura total aprox. 741cm, altura 112cm. Aproximadamente 10 portas de giro com moldura provençal. Armário com vidro transparente. 2 portas de embutir sistema Sotello FGVTN ou Häfele Slido F-Park72 50A (ocultação lava-louças e ice maker). Revestimento de coifa em MDF. Puxador alça preto 15 a 20cm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha – Balcão Inferior",desc:"Balcão em MDF Areia ou Off White, largura total aprox. 532cm, altura 78cm + sapata 15cm. Seções: 25 | 87 | 74,5 | 74,5 | 47,9 | 67,6 | 104 | 52cm. Aproximadamente 8 portas de giro com moldura provençal. Gavetas com corrediça oculta Hettich. Armário de canto (frente 85cm). Frentes descendo até o chão (113+120+80cm) para disfarçar porta de passagem. 2ª alzada inferior com portas de giro (widths 113+120+80cm, h=287cm). Puxador alça preto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha – Ilha / Bancada",desc:"Ilha central em MDF Jequitibá (Arauco), 300,8cm de largura. 2 fruteiras com frente em vidro incolor. Suporte para banquetas. Parte do conjunto com cooktop embutido.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha – Armário de Canto / Cristaleira / Despenseiro",desc:"Armário de canto com cristaleira: prateleiras em vidro, LED nas duas laterais (acionamento ao abrir). Despenseiro de extração frontal Häfele com 5 prateleiras de encaixe, prateleira de madeira, pintura eletrostática preta antrácito. 3 gavetas metálicas AvantBox Slim Baixa (FGVTN) + manta antiderrapante. 2 portas de embutir. 1 tomada para cafeteira e eletros. Seções: 43 | 67 | 100cm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha – Corpo Armário Lavanderia (Integrado)",desc:"Corpo de armário da lavanderia integrado internamente ao armário da cozinha, 313cm de largura. Inclui área para máquinas com abertura 50cm superior e inferior.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Lavanderia – Armários e Bancada",desc:"Armários em MDF Areia (Guararapes) ou Off White (Duratex) com moldura provençal. Armário 1 (206cm, h=70cm inf + 35cm sup): 4 portas superiores + 2 portas inferiores com gavetas. Armário 2 (235cm): 4 portas superiores (h=105cm) + abertura máquinas (h=90cm) + 2 portas inferiores (h=50cm) + tulha para roupa suja com fechamento e cesto. Armário 3 (77,8cm, h=235cm): 2 portas – prateleiras removíveis para acesso ao quadro de energia. Armário 4 (77,8cm, h=210cm): 2 portas. Cabideiro basculante Häfele (45×27cm). Puxador alça preto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Hóspede – Guarda-Roupa e Painel",desc:"Guarda-roupa em MDF Grafite Trama (Duratex) e MDF Jequitibá (Arauco). Ala esquerda (80+174cm, h=212cm): 6 portas de giro, puxador usinado no MDF. Ala direita (300cm, h=210cm): 4 portas de correr por trás do painel, 1 gaveteiro com puxador cava no MDF. Puxador basculante e gavetas: frente passante.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Circulação Superior – Painel e Armários",desc:"Painel ripado em MDF Areia ou Off White (759cm × 300cm) com vãos arredondados em módulos de aprox. 40cm. Pórtico em MDF ou gesso com iluminação indireta LED (acionamento na marcenaria). Armário alto central (110cm) com fechamento do quadro de energia (prateleiras removíveis). Prateleiras abertas nos nichos. Cantos arredondados. Armário alto lateral (h=220cm). 1 tomada elétrica embutida. MDF Jequitibá nos nichos internos. Puxador usinado no MDF.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Ana Laura – Cabeceira / Rack",desc:"Rack flutuante em MDF Grafite Trama (Duratex), 339×78cm, com reforço para não envergar. Frente passante sem puxador. 3 gavetas ocultas. Tomadas alocadas atrás do móvel. Espaço reservado para cortina e espelho no trilho.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Ana Laura – Closet",desc:"Closet em MDF Branco Diamante Trama (Duratex). Módulo 2 (213+60=273cm, h=245cm): 4 portas de giro com moldura e vidro incolor, puxador ponto preto, sapateira inferior 25cm. Módulo 3 (65+123+50=238cm, h=275cm): 4 portas com moldura e vidro incolor + fundo falso com porta deslizante para cofre oculto. Módulo 4 (25+245cm, h=245cm): 2 portas de correr por trás do armário. Todos os cabideiros com LED Elegance 1601 preto (acionamento na marcenaria). Sapateira em toda a parte inferior.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"WC Ana Laura – Gabinete e Nicho",desc:"Gabinete em MDF Cinza Cobalto VEL (Berneck), 146cm largura × 120cm altura. 2 gavetas frontais + cuba. Armário superior (100cm): 1 porta em espelho orgânico com LED frontal. Nicho lateral revestido em MDF, profundidade 30cm, 166cm altura × 45cm. Puxador ponto preto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Bella – Rack / Painel",desc:"Rack flutuante em MDF Relva (Leo Madeiras) ou semelhante, 343×78cm, com reforço para não envergar. Frente passante sem puxador. Gavetas ocultas. Tomadas alocadas atrás do móvel. Espaço para cortina e espelho no trilho.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Bella – Closet",desc:"Closet em MDF Areia (Guararapes) ou Off White (Duratex) ou Aurora (Duratex), sem fundo. Módulo frontal (393cm × 260cm): cabideiros superiores + 4 gavetas com frentes em moldura provençal + sapateira inferior 25cm (mais profunda). Módulo lateral (240cm × 260cm): cabideiros + gavetas + sapateira. Prateleiras com pino oculto 3,6cm / 1,8cm. LED acionamento por botão na marcenaria. Metalon 30×30. Puxador ponto preto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"WC Bella – Gabinete e Nicho",desc:"Gabinete em MDF Cinza Cobalto VEL (Berneck), 150cm (90+60cm) × 120cm altura. 2 gavetas + gavetão roupa suja (h=50cm). Armário superior (100cm): 1 porta em espelho orgânico com LED frontal. Coluna lateral (166cm × 30cm profundidade) com nicho revestido. Puxador ponto preto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master – Painel Ripado e Penteadeira",desc:"Painel ripado em MDF Jequitibá (Arauco), 380cm × 290cm (instalar acima do rodapé). Penteadeira flutuante em MDF Areia ou Off White (190cm), frente passante, lateral arredondada, 4 rodízios. Gaveteiro volante com 3 gavetas. Porta corredora por trás do painel com batente em marcenaria. LED acionamento na marcenaria. Puxador ponto champagne (gaveteiro).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master – Closet Feminino",desc:"Closet feminino em MDF Areia ou Off White + MDF Jequitibá (Arauco), sem fundo. Armário espelhado (perfil preto, puxador Slim Reto preto): 160cm × 210cm, profundidade 15cm. Seção closet 200cm: cabideiros superiores + 5 gavetas com moldura provençal (75+48+75cm) + sapateira inferior mais profunda + calceiro deslizante em MDF. 1ª gaveta porta-gravatas. Cabideiros com LED Elegance 1601 preto (acionamento na marcenaria). Puxador alça preto 15 a 20cm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master – Closet Masculino",desc:"Closet masculino em MDF Areia ou Off White + MDF Jequitibá (Arauco), sem fundo. Módulo 4 (260cm, h=260cm): cabideiros + prateleiras 35cm profundidade + 5 gavetas com moldura provençal + sapateira sem sapata. Módulo 5 (280cm): 3 colunas de cabideiros + gavetas (65+185cm visíveis). Módulo 6 (160cm): cabideiros + gavetas. Módulo 7 (160cm): prateleiras abertas + gavetas. LED acionamento por botão no móvel. Pino oculto 3,6cm / prateleiras 1,8cm.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
  ...
  const importarOrcBrunaLeonardo=()=>{
    ...
    const ambs=[
      ...
      {id:uid(),nome:"Cozinha",desc:"MDF Frapê – Arauco, MDF Savana – Guararapes e Quartzo Bianco Storm.\n\nARMÁRIOS SUPERIORES: Largura total aprox. 540cm (seções: 65+70+217+83+15+90cm), altura 62cm. Múltiplas portas de abrir + 1 basculante. LED embaixo dos armários. Aquecedor de água a gás Rinnai E15 REU E150 FEHGN8 embutido.\n\nARMÁRIOS INFERIORES E BALCÃO: Bancada Quartzo Bianco Storm. Largura total: 370cm, altura 77cm + 10cm pé. Composição: gavetão + basculante + portas abrir + gavetão. Lava e seca Midea HealthGuard Smart 11kg embutida. Forno a gás Electrolux OEBGF e micro-ondas Electrolux MI41S embutidos.\n\nTORRE DE ARMÁRIOS: 107cm largura × 235cm altura. 2 portas de abrir centrais (200cm). Internamente: gaveta, gaveta, basculante, nicho, gaveta, gavetão, gavetão. Laterais com prateleiras ajustáveis: 2 colunas de 51cm cada + coluna central 63+63+63cm.\n\nBANCADA / ILHA COM COOKTOP: 250cm × 134cm × 82cm altura. Cooktop de indução Electrolux IE6LP embutido (200cm × 82cm seção principal + 50cm extensão lateral). Base: gaveta, gaveta, gaveta + 2 gavetões.",insumos:[],vi:0,valorTotal:0},
  ...
  const importarOrcRodrigo=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("rodrigo")&&c?.endereco?.toLowerCase().includes("royal park");});
    if(jaExiste){return showToast("Orçamento Rodrigo (Royal Park) já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase()==="rodrigo")?.id;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"Rodrigo",tel:"",email:"",endereco:"Edifício Royal Park — Piracicaba/SP (Projeto SPASSO LOFT — Mauro Rossi 19 98154-8087 / Karime Tejada 19 99138-9632)",doc:""}]);}
    const ambs=[
      {id:uid(),nome:"Cozinha",desc:"Cozinha completa em MDF (cor a definir) — armários superiores + gabinete inferior com bancada.\n\nARMÁRIOS SUPERIORES:\nLargura total aprox. 540cm divididos em módulos de 89 + 101 + 101 + 101 + 72 + 76cm.\nAltura 59cm.\n12 portas de abrir (passantes, sem puxador aparente).\nEspaço reservado para coifa e nicho central de 160cm (para forno/micro/cafeteira) com tomadas embutidas.\nIluminação LED inferior nos armários sobre a bancada.\nCabideiro lateral em metalon (76cm).\n\nGABINETE INFERIOR E BANCADA:\nBalcão inferior em MDF sobre sapata 15cm.\nLargura total 305cm + extensão 381cm.\nMódulos: 53 + 63 + 50 + 62 + 69cm (com cuba sob bancada) + 73cm de torre lateral.\nAltura 70cm.\nComposição interna: gavetões (3 frentes), portas de abrir e nichos para forno embutido, máquina de lavar/secar e cooktop.\nBancada em pedra (quartzo ou similar) recobrindo toda a extensão.\nVão sem sapata reservado para máquina/lava-louças.\nFrentes mimetizadas (mesma altura) inclusive para porta de passagem.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha — Painel Ripado e Cabideiro",desc:"Painel ripado vertical em MDF amadeirado na lateral da cozinha (próximo à geladeira/lavanderia), seguindo \"na vira\" para a parede adjacente.\nCabideiro horizontal em metalon preto/dourado fixado sobre o painel ripado para acessórios e utensílios.\nAcabamento estendendo-se até o teto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Lavanderia — Coluna Integrada",desc:"Coluna em MDF (mesmo padrão da cozinha) integrada ao layout da cozinha, com profundidade 24cm e largura 79cm.\nEspaço para máquina de lavar com portas de abrir superiores (24cm) e inferiores.\nVentilação cruzada nas portas inferiores.\nFrente passante mimetizada com o restante da bancada.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala TV — Painel Ripado com Balcão de Pedra (Vista 2)",desc:"Painel ripado vertical em MDF amadeirado sobre parede pintada verde escuro.\nLargura total 425cm.\nBalcão de pedra (quartzo ou granito) com 33cm de altura sustentado em todo o comprimento.\nEmbaixo do balcão: módulos de gavetas e portas de abrir nas medidas 80 + 80 + 80 + 40 + 70 + 70cm (4 portas abrir + 2 gavetões + 2 gavetas internas).\nAltura interna do balcão 70cm + 5cm de moldura superior.\nPainel ripado vira para a parede adjacente (continuidade do acabamento).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala TV — Painel Ripado com TV (Vista 3)",desc:"Painel ripado vertical em MDF amadeirado, largura total 382cm × altura 220cm (com vira de 15cm).\nRecorte central de 142cm × 186cm para encaixe de TV de 65/75\" com cabeamento embutido.\nLateral direita com ripado 77 + 77 + 77cm.\nBalcão de pedra inferior 235cm × 50cm com 5 portas de abrir + 2 gavetas (frentes passantes sem puxador).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto 01 — Roupeiro em L",desc:"Roupeiro em \"L\" em MDF (cor a definir).\nLateral 1: 141cm × 214cm altura × 65cm profundidade.\nLateral 2 (frente): 76cm × 214cm altura × 50cm profundidade.\nComposição: Vista 1 — 2 portas de abrir superiores (63 + 72cm) + gaveteiro inferior com 5 gavetas (32 + 32 + 32cm) e 2 prateleiras finais.\nVista 2 — 2 portas de correr (164cm + 50cm) revestidas com porta de espelho na de 50cm.\nInterior: 6 prateleiras ajustáveis (28cm cada) na coluna estreita + área de cabideiro (122cm) + sapateira inferior.\nPino oculto, puxador cava.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto 01 — Bancada de Estudo / Penteadeira com Painel",desc:"Bancada flutuante em MDF (cor a definir) com tampo 185cm × 50cm profundidade.\nGaveteiro lateral acoplado com 4 gavetas (130cm largura × 75cm altura).\nPainel suspenso acima da bancada com 2 prateleiras (40cm e 92cm) com recorte para fita de LED Elegance embutida (acionamento na marcenaria).\nFrente das gavetas passante (sem puxador).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto 01 — Cabeceira e Mesa de Cabeceira",desc:"Cabeceira estofada/marcenaria 178cm largura × 115cm altura, com 2 gavetas internas centrais (50cm × 44cm) para guardar pertences.\nMesa de cabeceira flutuante com rodízios em MDF: 40cm × 50cm × 44cm altura, 2 gavetas frontais.\nEspelho oval lateral (decoração — não inclui marcenaria).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Casal — Painel Ripado com TV",desc:"Painel ripado vertical em MDF amadeirado, largura 262cm × altura 248cm (com vira superior 25cm).\nRecorte central para TV com cabeamento embutido.\nLateral direita com 3 prateleiras de 39cm de profundidade (alturas 22 + 22 + 18cm) para decoração.\nAcabamento estendendo até o teto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Casal — Armário sob Janela (Bancada)",desc:"Bancada/armário sob janela em MDF (cor a definir).\n247cm largura × 40cm profundidade × 80cm altura (com 14cm para gavetas).\nComposição: 2 gavetas superiores (40 + 40cm) + 4 portas de abrir (65 + 65 + 65 + 33cm) na parte inferior.\nTampo 247cm × 40cm para uso como bancada/aparador.\nFrentes passantes sem puxador.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Casal — Cabeceira e Mesas de Cabeceira",desc:"Cabeceira estofada/marcenaria 270cm largura × 115cm altura (largura total do quarto), com 2 gavetas internas (50cm × 44cm).\n2 mesas de cabeceira flutuantes com rodízios em MDF (40cm × 50cm × 44cm altura, 2 gavetas cada).\nFrente passante sem puxador.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Casal — Roupeiro em L com Nicho Vazado",desc:"Roupeiro em \"L\" em MDF (cor a definir).\nVista 1: 152cm × 276cm altura (50 + 37 + 65cm em profundidade variável).\nComposição — 2 portas superiores de abrir + nicho vazado central (decoração com LED) + 5 gavetas inferiores.\nVista 2: 245cm × 276cm altura × 50cm profundidade, com 3 portas de correr (frente lisa).\nInterior: cabideiros (4 colunas) + 5 prateleiras (29 + 30 + 30 + 29cm) + 5 gavetas (35 + 32 + 32 + 32 + 32 + 32cm em coluna) com puxador cava.\nPino oculto regulável.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Social — Gabinete e Armário Aéreo com Espelho",desc:"Gabinete inferior em MDF: 145cm largura × 49cm altura × 49cm profundidade, com 2 portas de abrir e cuba semi-embutida (75cm × 25cm reservados).\nBancada em pedra.\nArmário aéreo: 145cm × 101cm altura × 32cm profundidade.\nComposição: 2 portas de abrir laterais (63cm) com 3 prateleiras internas (30cm cada) + porta central em espelho orgânico (78cm) com puxador discreto.\nFaixa de 44cm acima da bancada para azulejos/decoração.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Suíte — Gabinete e Armário Aéreo com Espelho",desc:"Gabinete inferior em MDF: 164cm largura × 45cm altura × 60cm profundidade, com 2 portas de abrir e cuba semi-embutida (78cm × 25cm reservados).\nBancada em pedra.\nArmário aéreo: 165cm × 101cm altura × 32cm profundidade.\nComposição: 2 portas de abrir laterais (76cm) com 3 prateleiras internas (30cm cada) + porta central em espelho orgânico (84cm) com puxador discreto.\nFaixa de 44cm acima da bancada para azulejos/decoração.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
  ...
  const importarOrcCarina=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase()==="carina";});
    if(jaExiste){return showToast("Orçamento Carina já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase()==="carina")?.id;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"Carina",tel:"",email:"",endereco:"Rua Orlando Baltieri — Piracicaba/SP",doc:""}]);}
    const ambs=[
      {id:uid(),nome:"Cozinha",desc:"Cozinha completa em MDF (cor a definir) — armários superiores + gabinete inferior.\nPortas lisas (sem moldura, sem ripado, sem puxador embutido).\nPuxador fino champanhe (perfil slim horizontal) em todas as portas e gavetas.\n\nARMÁRIOS SUPERIORES:\nMódulos sob teto/laje, altura aprox. 70cm.\nPortas de abrir lisas com puxador slim champanhe.\nNicho central para forno/micro-ondas/cafeteira embutidos.\nIluminação LED inferior nos armários sobre a bancada (sancas perimetrais).\n\nGABINETE INFERIOR:\nBalcão inferior em MDF sobre sapata.\nAltura padrão 70cm.\nComposição: gavetões (3 frentes em coluna), portas de abrir lisas e nicho ventilado para forno embutido.\nFrente para máquina de lava-louças e cooktop conforme posicionamento da planta.\nFrentes mimetizadas (mesma altura) inclusive para porta de passagem se houver.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala — Painel TV (Estilo Foto de Inspiração)",desc:"Painel de TV em MDF (cor a definir, tom areia/bege claro conforme foto de inspiração).\nLayout em 3 partes:\n— Painel central liso (sem ripado) com recorte para TV e cabeamento embutido.\n— Painel ripado vertical lateral (lado direito) em MDF amadeirado.\n— Recesso superior com fita de LED indireta (efeito halo no teto).\nBalcão flutuante inferior corrido em MDF (cor combinando com o painel central), com 4 gavetas frente passante (sem puxador) e tampo extensível.\nFita de LED também sob o balcão (efeito flutuante).\nAcionamento dos LEDs por interruptor na marcenaria.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala de Jantar — Cristaleira",desc:"Cristaleira em MDF (cor a definir).\nPortas de vidro transparente com armação fina em alumínio champanhe (perfil slim).\nPuxador fino champanhe (perfil slim vertical) combinando com a armação.\nPrateleiras internas em vidro com fita de LED Elegance embutida (acionamento na marcenaria).\nFundo do móvel revestido em espelho ou MDF amadeirado (a definir com cliente).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Área Gourmet — Armários e Adega",desc:"Armário gourmet em MDF (cor a definir).\nArmários superiores com portas lisas de abrir + puxador slim champanhe.\nGabinete inferior com gavetões e portas lisas (mimetizadas).\nNicho central reservado para cooktop, churrasqueira ou adega conforme planta.\nIluminação LED inferior nos armários superiores.\nFrentes lisas e puxador slim champanhe em todas as partes.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Lavanderia",desc:"Armários da lavanderia em MDF (cor a definir).\nSuperiores: portas de abrir lisas com puxador slim champanhe.\nPrateleiras internas reguláveis (pino oculto).\nGabinete inferior com nicho ventilado para máquina de lavar/secar e tanque.\nPortas de abrir + 1 gavetão para roupas sujas (tulha embutida com cesto removível).\nFrentes lisas mimetizadas.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Porta Pivotante — Área Íntima",desc:"Porta pivotante em MDF lisa (sem ripado, sem moldura, sem detalhes).\nAcabamento na mesma cor da marcenaria ambiente ou definido pelo cliente.\nFerragem pivotante oculta tipo Italyana ou similar.\nPuxador slim champanhe vertical combinando com o restante dos móveis.\nFunção: separar a área social da área íntima dos dormitórios.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master — Mesas de Cabeceira",desc:"2 mesas de cabeceira em MDF madeira lisa (cor a definir).\nFrente totalmente lisa em madeira (desconsiderar palha do projeto original).\nFormato suspenso/flutuante ou apoiado em pé fino conforme escolha do cliente.\nDimensões aprox. 50cm × 40cm × 40cm altura.\n2 gavetas com frente passante (sem puxador) — abertura por frente passante ou tip-on.\nObs.: Suíte master SEM cabeceira de marcenaria (conforme solicitação do cliente).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master — Closet com Painel de Espelho (Estilo Foto)",desc:"Closet em MDF (cor a definir).\nFrente em painel espelhado modular conforme foto de inspiração:\n— Espelhos verticais com perfil de LED entre eles (efeito espelho + iluminação contínua vertical).\n— Acionamento dos LEDs por sensor ou botão na marcenaria.\nInterior: cabideiros (perfil metalon dourado/champanhe) + prateleiras reguláveis (pino oculto) + gaveteiro com 5 gavetas frente passante.\nSapateira inferior com prateleiras inclinadas.\nIluminação LED Elegance nos cabideiros (acionamento ao abrir).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Master — Gabinete e Espelhos com LED Vertical",desc:"Gabinete inferior em MDF (cor a definir).\nFrente lisa com puxador slim champanhe.\nLargura conforme bancada (definida pela pedra).\n2 gavetões frontais + portas de abrir laterais.\nESPELHO COM LED VERTICAL (estilo foto de inspiração):\n— Painel espelhado dividido em módulos com perfil de LED vertical entre eles.\n— Iluminação contínua de cima até a bancada.\n— Centro do conjunto com nicho aberto revestido em MDF amadeirado + 3 prateleiras de vidro com LED inferior.\nAcionamento dos LEDs por sensor ou botão na marcenaria.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Social — Gabinete e Espelho",desc:"Gabinete inferior em MDF (cor a definir).\nFrente lisa com puxador slim champanhe.\n2 portas de abrir + 1 gaveta superior.\nEspelho de bancada com perfil de LED contornando (acionamento na marcenaria).\nNicho lateral em MDF (se houver espaço) para guarda de toalhas e perfumaria.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Suíte 01 — Gabinete e Espelho",desc:"Gabinete inferior em MDF (cor a definir).\nFrente lisa com puxador slim champanhe.\n2 portas de abrir + 1 gaveta superior.\nEspelho de bancada com perfil de LED contornando (acionamento na marcenaria).\nObs.: Suíte 01 SEM armário de roupas (conforme solicitação do cliente).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Suíte 02 — Gabinete e Espelho",desc:"Gabinete inferior em MDF (cor a definir).\nFrente lisa com puxador slim champanhe.\n2 portas de abrir + 1 gaveta superior.\nEspelho de bancada com perfil de LED contornando (acionamento na marcenaria).\nObs.: Suíte 02 SEM armário de roupas (conforme solicitação do cliente).",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
  ...
  const importarOrcDenisCintya=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("denis")&&c?.nome?.toLowerCase().includes("cintya");});
    if(jaExiste){return showToast("Orçamento Denis e Cintya já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("denis")&&c.nome?.toLowerCase().includes("cintya"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"Denis e Cintya",tel:"",email:"",endereco:"",doc:""}]);}
    const ambs=[
      {id:uid(),nome:"Cozinha",desc:"Cozinha completa em MDF (cor a escolher) — armários superiores + gabinete inferior + torre da geladeira.\n\nARMÁRIOS SUPERIORES:\nLargura conforme planta, altura aprox. 70cm.\nPortas de abrir lisas.\nNicho central para micro-ondas e nicho para purificador de água.\nIluminação LED inferior nos armários sobre a bancada (efeito frisado/ripado claro no fundo).\n\nGABINETE INFERIOR:\nBalcão inferior em MDF sobre sapata.\nAltura padrão 70cm.\nComposição: gavetões (3 frentes em coluna), portas de abrir lisas e nicho ventilado para forno embutido.\nNicho para lava-louças embutida.\nFrente para cooktop com gás integrado à bancada.\nFrentes mimetizadas e cantos arredondados.\n\nTORRE DA GELADEIRA:\nColuna lateral em MDF que faz acabamento ao redor da geladeira inox preta (sem tampar a frente).\nLargura aprox. 80cm com fechamento superior até o teto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala — Painel TV e Rack Suspenso",desc:"Painel de TV em MDF (cor a escolher) na parede principal da sala.\nLargura conforme parede.\nAltura piso ao teto.\nRecorte central para TV com cabeamento embutido.\n\nRACK SUSPENSO INFERIOR:\nRack flutuante em MDF (cor a escolher) com 2 ou 3 portas de abrir.\nFrente passante (sem puxador aparente).\nIluminação LED indireta sob o rack (efeito flutuante).\nTampo extensível para apoio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala / Jantar — Painel Ripado Divisor",desc:"Painel ripado vertical em MDF amadeirado escuro (tom cacau/grafite — cor a escolher) como divisória/decoração entre sala de estar e sala de jantar.\nRipas com frisos verticais (ripa 2cm / friso 2cm — padrão).\nAcabamento estendendo do piso até o teto.\nCantoneira metálica preta em L na base para proteção do painel.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala de Jantar — Armário Buffet com Nicho e Cristaleira",desc:"Armário alto em MDF (cor a escolher) com nicho central revestido em madeira amadeirada (tom mel/carvalho).\n\nMÓDULO PRINCIPAL:\n4 portas de abrir lisas mimetizadas (sem puxador aparente) — superiores e inferiores.\nNicho horizontal central revestido em MDF amadeirado para decoração ou bar.\nFerragens com amortecimento.\n\nMÓDULO LATERAL (cristaleira):\nArmário lateral com porta de vidro temperado incolor com armação fina em alumínio (cor a escolher — preta ou champanhe).\nPrateleiras internas com iluminação LED Elegance.\nPuxador slim sobreposto combinando com a armação.\nFundo do nicho em MDF amadeirado.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha — Banco Janela com Tampo de Madeira",desc:"Banco fixo sob a janela em MDF (cor a escolher).\nTampo extensível em MDF amadeirado (tom mel — cor a escolher) que serve também como mesa lateral/aparador.\nAssento estofado em duas almofadas (tecido a escolher).\nBase com gavetão inferior para guardar utensílios.\nDimensão conforme janela.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Lavanderia / Área de Serviço",desc:"Móvel completo para área de serviço em MDF (cor a escolher).\n\nARMÁRIOS SUPERIORES:\nPrateleiras superiores fixas em MDF (cor a escolher) com fixação oculta por grapas.\nCabideiro horizontal em metalon preto para varal de roupas.\n\nGABINETE INFERIOR:\nGabinete sob a bancada com portas de abrir lisas.\nNicho ventilado para máquina de lavar/secar.\nNicho lateral para tanque + cuba (espaço para 2 cubas conforme planta).\nGavetão lateral para roupa suja (tulha embutida).\nFrentes lisas mimetizadas.\n\nDIVISOR / NICHO AQUECEDOR:\nColuna estreita para aquecedor de passagem (Lorenzetti ou similar) com fechamento superior.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Despensa — Prateleiras em \"L\"",desc:"Despensa em MDF (cor a escolher) com aproveitamento de canto.\nPrateleiras em formato \"L\" em 4 ou 5 níveis (a definir conforme altura).\nFixação oculta por grapas / suporte interno.\nAcabamento liso.\nProfundidade aprox. 30 a 40cm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master — Painel Cabeceira Estofado",desc:"Painel de cabeceira da cama em MDF (estrutura) revestido com estofado em tecido (cor e modelo a escolher).\nFormato vertical canelado (gomos verticais largos).\nLargura conforme cabeceira da cama 2,30m.\nAltura aprox. 1,40m a 1,50m.\nFixação em parede.\nObs.: papel de parede listrado complementar (decoração — não inclui marcenaria).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master — Armário e Bancada sob a Janela",desc:"Conjunto em MDF (cor a escolher) sob a janela do quarto principal.\n\nMÓDULO LATERAL ESQUERDO:\nArmário alto com 1 porta de abrir + nichos internos abertos com prateleiras de vidro temperado (para decoração).\nLargura aprox. 50cm.\n\nBANCADA CENTRAL (sob janela):\nBancada flutuante em MDF (cor a escolher) — funciona como penteadeira/escrivaninha.\nGaveteiro acoplado com 4 gavetas lisas (frente passante, sem puxador).\nTampo apoio para uso como penteadeira.\n\nMÓDULO LATERAL DIREITO:\nColuna estreita com 4 nichos abertos pequenos em madeira amadeirada (tom mel) para decoração / cosméticos.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte Master — Roupeiro Principal (8 Portas)",desc:"Roupeiro principal em MDF (cor a escolher) com 8 portas de giro em fileira.\nLargura total aprox. 4,00 a 4,50m (conforme parede do quarto/closet).\nAltura piso ao teto.\nPuxadores verticais finos em alumínio escovado.\n\nINTERIOR:\nCabideiros longos (varões) em 3 colunas com araras superiores.\nPrateleiras ajustáveis (pino oculto).\n2 colunas de gavetas internas com 4 gavetas cada (frentes passantes).\nSapateira inferior com prateleiras.\nPortas com amortecimento + ferragens com amortecimento.\n\nMÓDULO LATERAL CRISTALEIRA:\nColuna lateral com 2 portas de vidro temperado incolor com armação fina (perfil slim alumínio — cor a escolher) — para guardar bolsas, sapatos especiais ou decoração.\nPrateleiras internas em vidro com iluminação LED.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Suíte Master — Gabinete e Espelho",desc:"Gabinete inferior suspenso em MDF (cor a escolher — cinza grafite no projeto).\nFrente lisa com puxador slim.\n2 gavetões frontais + 1 porta lateral.\nNicho aberto sob a bancada para apoio.\nLargura conforme bancada de pedra.\n\nESPELHO:\nEspelho de bancada largura total com perfil de LED frontal contornando (acionamento na marcenaria).\nFaixa entre bancada e espelho revestida em MDF.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto Filha — Armário 2 Portas",desc:"Armário em MDF (cor a escolher — branco no projeto).\n2 portas de abrir lisas com puxadores decorativos em formato de borboleta (puxadores a escolher pela cliente).\nLargura aprox. 1,20m × altura piso ao teto.\nPortas com amortecimento.\n\nINTERIOR:\nCabideiro superior (varão) + prateleiras ajustáveis (pino oculto).\nGavetas internas (a definir quantidade).\nFundo em MDF.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto Hóspede — Cabeceira com Nicho",desc:"Painel de cabeceira da cama em MDF (cor a escolher) com nicho horizontal central revestido em MDF amadeirado (tom mel — cor a escolher).\nLargura conforme cabeceira da cama.\nAltura aprox. 50cm × espessura 4cm.\nFixação em parede.\nDesign minimalista (frisado horizontal apenas no nicho).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho Social — Gabinete e Espelho",desc:"Gabinete inferior suspenso em MDF (cor a escolher).\nFrente lisa com puxador slim.\n2 gavetões frontais.\nLargura conforme bancada de pedra.\n\nESPELHO:\nEspelho de bancada com perfil de LED contornando (acionamento na marcenaria).\nFormato retangular.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto / Escritório — Bancada de Estudo e Prateleiras",desc:"Bancada/escrivaninha em MDF (cor a escolher) com tampo em MDF amadeirado (tom mel — cor a escolher).\nLargura conforme parede.\nGaveteiro acoplado com 2 a 3 gavetas (frente passante).\n\nPRATELEIRAS SUPERIORES:\n2 prateleiras suspensas em MDF amadeirado com fixação oculta por grapas.\nEspessura 4cm × profundidade 30cm.\n\nARMÁRIO LATERAL:\nColuna alta de armário com 1 porta de abrir lisa (cor a escolher) + puxador slim em madeira amadeirada.\nFerragens com amortecimento.\n\nARMÁRIOS SUPERIORES (sobre prateleiras):\nLinha de armários aéreos com portas de abrir lisas para guarda de livros e materiais.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
  ...
  const importarOrcSodero=()=>{
    const ambsBase=[
      {nome:"Cozinha",desc:"ARMARIOS SUPERIORES:\nArmários superiores 3300x900x450 + 800x500x450 (em cima da geladeira) em MDF Freijó Guararapes — portas de abrir lisas, frentes mimetizadas, dobradiças com amortecimento.\nIluminação em LED.\nPuxador cava na madeira.\n\nGABINETE INFERIOR:\nGabinete inferior 1500x720x650 + 630x720x650 em MDF Gianduia Trama Duratex — composição com gavetas (4 frentes em coluna), portas de abrir, nicho para lava-louças, e espaçamento para maquina de lavar.\nPuxador cava na madeira.\n\nILHA CENTRAL:\nIlha central 2150x670x720 em MDF Gianduia com 2 gavetas e 1 gavetão, fechamneto para forno embutido.\nRevestimento frente.\nNichos decorativos na lateral.\nInterior em MDF Branco Tx.\nPuxador cava na madeira.\n\nARMÁRIO BAR: 100x720x400 (inferior).\nArmario com portas de abrir.\nPrateleira interna.\nPuxador cava na madeira."},
      {nome:"Cozinha — Prateleiras",desc:"PRATELEIRAS SUPERIORES:\n3 prateleiras em Freijó 1000x40x300.\n\nCRISTALEIRA SUSPENSA:\nMódulo suspenso 700x1140x300 em MDF Gianduia Trama Duratex.\nPortas com vidro temperado incolor e moldura em MDF.\nFundo revestido em espelho.\nPrateleiras internas em vidro temperado.\nIluminação em LED."},
      {nome:"Varanda Gourmet",desc:"ARMÁRIOS INFERIORES:\nGabinete inferior 1700x700x650 em MDF Gianduia Trama Duratex.\nComposição:4 Portas de giro e 2 gavetões + Portas superiores ventiladas e fechamento para churrasqueira embutida.\n\nTORRE:\nColuna 850x2200x400 em MDF Gianduia para utensílios e armazenagem.\nPortas de abrir lisas + prateleiras internas.\nInterior em MDF Branco TX.\n\nCACHEPOT: 600x900x350.\n2 Unidades em MDF Freijó Guararapes com pés em metalon (pintura eletrostatica preto fosco)."},
      {nome:"Sala de Estar — Painel TV e Rack",desc:"Painel de TV Frisado em L 3800x2450x40 em MDF Freijó Guararapes.\nIluminação indireta e LED.\nPrateleira superior 6250x350x40.\n\nRACK INFERIOR FLUTUANTE:\nRack suspenso 1900x400x350 em MDF Gianduia Trama Duratex.\n2 gavetas com frente passante (sem puxador aparente)."},
      {nome:"Quarto — Costura e Visitas",desc:"CAMA FUNCIONAL SOLTEIRO:\nCama de solteiro 850x350x1950 em MDF Freijó Guararapes.\n3 gavetões inferiores para armazenagem.\n\nBAÚ CABECEIRA:\nBaú/painel cabeceira 850x800x500 em MDF Freijó Guararapes com tampa basculante + pistão a gás e 2 gavetas.\n\nBANCADA DE COSTURA:\nBancada de trabalho 1870x400x500 em MDF Freijó Guararapes.\narmário embaixo da bancada com 1 porta de giro e prateleira interna 600x780x600.\n\nARMÁRIO SUPERIOR:\nArmário com 1 porta de abrir, prateleira interna 850x1150x320.\nPrateleiras abertas superiores 2200x40x250 em MDF Freijó — fixação oculta por grapas com perfil de LED embutido por baixo.\n3 prateleiras em L de 1230x40x300+600x40x500.\n1 de 600x40x500.\n\nGUARDA ROUPA:\nArmário 1000x2550x600 em MDF Gianduia Trama Duratex.\nDivisão entre área de roupas (cabideiro + gavetas internas + sapateira inferior) e área de costura (prateleiras + gavetas para acessórios).\nPrateleiras superiores.\nFundo em MDF Branco tx.\nInterior em MDF Branco TX.\nPuxador slim em alumínio."},
      {nome:"Quarto da Filha",desc:"CAMA CASAL COM GAVETAS:\nCama casal 1430x1930x350 em MDF Freijó Guararapes.\nCom gavetões frontais (2 frentes em coluna).\n4 gavetas, 2 em cada lado.\n\nCABECEIRA EM MADEIRA:\nCabeceira 2800x1250x40 em MDF Freijó Guararapes com perfil de LED embutido por trás (iluminação indireta).\nFixação em parede.\n\nCRIADOS-MUDOS (2 unidades):\nCriado-mudo 650x550x450 em MDF Freijó Guararapes (estrutura) com frentes de gaveta em MDF Gianduia Trama Duratex.\n2 gavetas com frente passante.\n\nGUARDA-ROUPA / CLOSET (3 portas de correr):\nArmário 2900x2460x600 em MDF Gianduia Trama Duratex.\n3 portas de correr — 1 central espelhada + 2 laterais em acabamento Gianduia.\nTrilho superior embutido (sistema slim).\n\nINTERIOR DO CLOSET (em MDF Branco TX):\n3 cabideiros em alturas diferentes (0,95m / 1,05m / 1,20m) em tubo oblongo cromado.\nCabideiro lateral para roupas longas.\nGavetas internas — coluna esquerda (5 gavetas), coluna central (4 sapateiras) e coluna direita (4 gavetas).\nPrateleiras superiores para caixas e malas (alturas 0,25 / 0,30 / 0,57m).\nSapateira inferior com prateleiras.\nFundo em MDF Branco tx.\n\nPAINEL DE TV:\nPainel TV 1150x2460x40 em MDF Freijó Guararapes.\nPrateleira horizontal 2050x40x250 em MDF Gianduia Trama Duratex com fixação oculta por grapas."},
      {nome:"Suíte Casal",desc:"CAMA QUEEN COM GAVETAS:\nCama queen 1600x400x2000 em MDF Freijó Guararapes.\nCom gavetões frontais (2 frentes em coluna).\n2 gavetões e 1 gaveta menor de cada lado.\n\nCABECEIRA EM FREIJÓ:\nPainel cabeceira 2800x1250x40 em MDF Freijó Guararapes — fixação em parede com perfil de LED embutido na borda superior (iluminação indireta).\n\nCRIADOS-MUDOS (2 unidades):\nCriado-mudo 530x400x500 em MDF Freijó Guararapes com rodízios escondidos.\n2 gavetas + 1 nicho aberto inferior.\nFrente passante puxador cava.\n\nARMÁRIO 4 PORTAS DE CORRER COM TV:\nArmário planejado 3800x2560x600 em MDF Gianduia Trama Duratex.\n4 portas de correr em Reflecta Bronze (efeito espelhado bronze) com perfil slim Bronze.\nTrilho superior embutido (sistema slim) — abertura suave com amortecimento.\nRecorte para TV fixada na 3ª porta (alinhada à cama) com cabeamento embutido.\n\nINTERIOR DO ARMÁRIO (em MDF Branco TX):\nCabideiros em diferentes alturas (1,05m e 1,95m) em tubo oblongo cromado para roupas curtas e longas.\nColuna esquerda — gaveteiro com 4 gavetas + sapateira inferior com 3 prateleiras.\nColunas centrais — cabideiros + gaveteiro com 5 gavetas (acessórios e roupas dobradas).\nColuna direita — gaveteiro com 3 gavetas + sapateira inferior + prateleiras superiores.\nPrateleiras superiores corridas para caixas e malas."},
      {nome:"Banheiro Suíte",desc:"GABINETE INFERIOR SUSPENSO:\nGabinete suspenso 800x700x500 em MDF Gianduia.\n1 Basculante e 1 Gaveta com corrediças invisíveis e puxador cava.\n\nESPELHEIRA SUSPENSA:\nArmário espelheira 800x900x150 em MDF Gianduia com 3 portas espelhadas.\nPrateleiras internas em MDF branco tx para armazenagem de itens de higiene.\nDobradiças em inox com amortecimento.\nFita de LED embutida na borda inferior (iluminação direta sobre a bancada).\no led se extente embaixo da prateleira tb.\n\nPRATELEIRA DE APOIO:\nPrateleira lateral 600x400x150 em MDF Branco Tx com fixação oculta por grapas para apoio de plantas e decoração.\n\nFundo em MDF Branco TX.\nPuxador cava na madeira."},
      {nome:"Banheiro Social",desc:"GABINETE INFERIOR SUSPENSO:\nGabinete suspenso 800x700x500 em MDF Gianduia.\n1 Basculante e 1 Gaveta com corrediças invisíveis e puxador cava.\n\nESPELHEIRA SUSPENSA:\nArmário espelheira 800x900x150 em MDF Gianduia com 3 portas espelhadas.\nPrateleiras internas em MDF branco tx para armazenagem de itens de higiene.\nDobradiças em inox com amortecimento.\nFita de LED embutida na borda inferior (iluminação direta sobre a bancada).\no led se extente embaixo da prateleira tb.\n\nPRATELEIRA DE APOIO:\nPrateleira lateral 600x400x150 em MDF Branco Tx com fixação oculta por grapas para apoio de plantas e decoração.\n\nFundo em MDF Branco TX.\nPuxador cava na madeira."},
    ];
    const cliExistenteId=clientes.find(c=>c.nome?.toLowerCase().includes("sodero"))?.id;
    const orcExistente=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("sodero");});
    if(orcExistente){
      const nomesExistentes=new Set((orcExistente.ambientes||[]).map(a=>a.nome));
      const novos=ambsBase.filter(a=>!nomesExistentes.has(a.nome)).map(a=>({id:uid(),nome:a.nome,desc:a.desc,insumos:[],vi:0,valorTotal:0}));
      if(novos.length===0){return showToast("Orçamento Sodero já tem todos os ambientes!","red");}
      updOrc(orcExistente.id,o=>({...o,ambientes:[...o.ambientes,...novos]}));
      setOrcAtivo(orcExistente.id);
      setTab("orcamentos");
      return showToast(`${novos.length} ambiente(s) adicionado(s) ao ${orcExistente.num}!`);
    }
    let cliId=cliExistenteId;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"Condomínio Sodero",tel:"",email:"",endereco:"",doc:""}]);}
    const ambs=ambsBase.map(a=>({id:uid(),nome:a.nome,desc:a.desc,insumos:[],vi:0,valorTotal:0}));
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
    setOrcamentos(p=>[...p,o]);
  ...
  const atualizarSoderoCorrecoes=()=>{
    const orcExistente=orcamentos.find(o=>{
      const c=clientes.find(x=>x.id===o.clienteId);
      return c?.nome?.toLowerCase().includes("sodero")||c?.nome?.toLowerCase().includes("okoti")||c?.nome?.toLowerCase().includes("rosa");
    });
    if(!orcExistente){return showToast("Orçamento Rosa/Sodero não encontrado","red");}
    const correcoes={
      "Cozinha":{newName:"Cozinha",newDesc:"ARMARIOS SUPERIORES:\nArmários superiores 3300x900x450 + 800x500x450 (em cima da geladeira) em MDF Freijó Guararapes — portas de abrir lisas, frentes mimetizadas, dobradiças com amortecimento.\nIluminação em LED.\nPuxador cava na madeira.\n\nGABINETE INFERIOR:\nGabinete inferior 1500x720x650 + 630x720x650 em MDF Gianduia Trama Duratex — composição com gavetas (4 frentes em coluna), portas de abrir, nicho para lava-louças, e espaçamento para maquina de lavar.\nPuxador cava na madeira.\n\nILHA CENTRAL:\nIlha central 2150x670x720 em MDF Gianduia com 2 gavetas e 1 gavetão, fechamneto para forno embutido.\nRevestimento frente.\nNichos decorativos na lateral.\nInterior em MDF Branco Tx.\nPuxador cava na madeira.\n\nARMÁRIO BAR: 100x720x400 (inferior).\nArmario com portas de abrir.\nPrateleira interna.\nPuxador cava na madeira."},
      "Cozinha — Nicho e Cristaleira Suspensa":{newName:"Cozinha — Prateleiras",newDesc:"PRATELEIRAS SUPERIORES:\n3 prateleiras em Freijó 1000x40x300.\n\nCRISTALEIRA SUSPENSA:\nMódulo suspenso 700x1140x300 em MDF Gianduia Trama Duratex.\nPortas com vidro temperado incolor e moldura em MDF.\nFundo revestido em espelho.\nPrateleiras internas em vidro temperado.\nIluminação em LED."},
      "Cozinha — Prateleiras":{newName:"Cozinha — Prateleiras",newDesc:"PRATELEIRAS SUPERIORES:\n3 prateleiras em Freijó 1000x40x300.\n\nCRISTALEIRA SUSPENSA:\nMódulo suspenso 700x1140x300 em MDF Gianduia Trama Duratex.\nPortas com vidro temperado incolor e moldura em MDF.\nFundo revestido em espelho.\nPrateleiras internas em vidro temperado.\nIluminação em LED."},
      "Varanda Gourmet":{newName:"Varanda Gourmet",newDesc:"ARMÁRIOS INFERIORES:\nGabinete inferior 1700x700x650 em MDF Gianduia Trama Duratex.\nComposição:4 Portas de giro e 2 gavetões + Portas superiores ventiladas e fechamento para churrasqueira embutida.\n\nTORRE:\nColuna 850x2200x400 em MDF Gianduia para utensílios e armazenagem.\nPortas de abrir lisas + prateleiras internas.\nInterior em MDF Branco TX.\n\nCACHEPOT: 600x900x350.\n2 Unidades em MDF Freijó Guararapes com pés em metalon (pintura eletrostatica preto fosco)."},
      "Sala de Estar — Painel TV e Rack":{newName:"Sala de Estar — Painel TV e Rack",newDesc:"Painel de TV Frisado em L 3800x2450x40 em MDF Freijó Guararapes.\nIluminação indireta e LED.\nPrateleira superior 6250x350x40.\n\nRACK INFERIOR FLUTUANTE:\nRack suspenso 1900x400x350 em MDF Gianduia Trama Duratex.\n2 gavetas com frente passante (sem puxador aparente)."},
      "Quarto Compacto — Costura e Visitas":{newName:"Quarto — Costura e Visitas",newDesc:"CAMA FUNCIONAL SOLTEIRO:\nCama de solteiro 850x350x1950 em MDF Freijó Guararapes.\n3 gavetões inferiores para armazenagem.\n\nBAÚ CABECEIRA:\nBaú/painel cabeceira 850x800x500 em MDF Freijó Guararapes com tampa basculante + pistão a gás e 2 gavetas.\n\nBANCADA DE COSTURA:\nBancada de trabalho 1870x400x500 em MDF Freijó Guararapes.\narmário embaixo da bancada com 1 porta de giro e prateleira interna 600x780x600.\n\nARMÁRIO SUPERIOR:\nArmário com 1 porta de abrir, prateleira interna 850x1150x320.\nPrateleiras abertas superiores 2200x40x250 em MDF Freijó — fixação oculta por grapas com perfil de LED embutido por baixo.\n3 prateleiras em L de 1230x40x300+600x40x500.\n1 de 600x40x500.\n\nGUARDA ROUPA:\nArmário 1000x2550x600 em MDF Gianduia Trama Duratex.\nDivisão entre área de roupas (cabideiro + gavetas internas + sapateira inferior) e área de costura (prateleiras + gavetas para acessórios).\nPrateleiras superiores.\nFundo em MDF Branco tx.\nInterior em MDF Branco TX.\nPuxador slim em alumínio."},
      "Quarto — Costura e Visitas":{newName:"Quarto — Costura e Visitas",newDesc:"CAMA FUNCIONAL SOLTEIRO:\nCama de solteiro 850x350x1950 em MDF Freijó Guararapes.\n3 gavetões inferiores para armazenagem.\n\nBAÚ CABECEIRA:\nBaú/painel cabeceira 850x800x500 em MDF Freijó Guararapes com tampa basculante + pistão a gás e 2 gavetas.\n\nBANCADA DE COSTURA:\nBancada de trabalho 1870x400x500 em MDF Freijó Guararapes.\narmário embaixo da bancada com 1 porta de giro e prateleira interna 600x780x600.\n\nARMÁRIO SUPERIOR:\nArmário com 1 porta de abrir, prateleira interna 850x1150x320.\nPrateleiras abertas superiores 2200x40x250 em MDF Freijó — fixação oculta por grapas com perfil de LED embutido por baixo.\n3 prateleiras em L de 1230x40x300+600x40x500.\n1 de 600x40x500.\n\nGUARDA ROUPA:\nArmário 1000x2550x600 em MDF Gianduia Trama Duratex.\nDivisão entre área de roupas (cabideiro + gavetas internas + sapateira inferior) e área de costura (prateleiras + gavetas para acessórios).\nPrateleiras superiores.\nFundo em MDF Branco tx.\nInterior em MDF Branco TX.\nPuxador slim em alumínio."},
      "Quarto da Filha":{newName:"Quarto da Filha",newDesc:"CAMA CASAL COM GAVETAS:\nCama casal 1430x1930x350 em MDF Freijó Guararapes.\nCom gavetões frontais (2 frentes em coluna).\n4 gavetas, 2 em cada lado.\n\nCABECEIRA EM MADEIRA:\nCabeceira 2800x1250x40 em MDF Freijó Guararapes com perfil de LED embutido por trás (iluminação indireta).\nFixação em parede.\n\nCRIADOS-MUDOS (2 unidades):\nCriado-mudo 650x550x450 em MDF Freijó Guararapes (estrutura) com frentes de gaveta em MDF Gianduia Trama Duratex.\n2 gavetas com frente passante.\n\nGUARDA-ROUPA / CLOSET (3 portas de correr):\nArmário 2900x2460x600 em MDF Gianduia Trama Duratex.\n3 portas de correr — 1 central espelhada + 2 laterais em acabamento Gianduia.\nTrilho superior embutido (sistema slim).\n\nINTERIOR DO CLOSET (em MDF Branco TX):\n3 cabideiros em alturas diferentes (0,95m / 1,05m / 1,20m) em tubo oblongo cromado.\nCabideiro lateral para roupas longas.\nGavetas internas — coluna esquerda (5 gavetas), coluna central (4 sapateiras) e coluna direita (4 gavetas).\nPrateleiras superiores para caixas e malas (alturas 0,25 / 0,30 / 0,57m).\nSapateira inferior com prateleiras.\nFundo em MDF Branco tx.\n\nPAINEL DE TV:\nPainel TV 1150x2460x40 em MDF Freijó Guararapes.\nPrateleira horizontal 2050x40x250 em MDF Gianduia Trama Duratex com fixação oculta por grapas."},
      "Suíte Casal":{newName:"Suíte Casal",newDesc:"CAMA QUEEN COM GAVETAS:\nCama queen 1600x400x2000 em MDF Freijó Guararapes.\nCom gavetões frontais (2 frentes em coluna).\n2 gavetões e 1 gaveta menor de cada lado.\n\nCABECEIRA EM FREIJÓ:\nPainel cabeceira 2800x1250x40 em MDF Freijó Guararapes — fixação em parede com perfil de LED embutido na borda superior (iluminação indireta).\n\nCRIADOS-MUDOS (2 unidades):\nCriado-mudo 530x400x500 em MDF Freijó Guararapes com rodízios escondidos.\n2 gavetas + 1 nicho aberto inferior.\nFrente passante puxador cava.\n\nARMÁRIO 4 PORTAS DE CORRER COM TV:\nArmário planejado 3800x2560x600 em MDF Gianduia Trama Duratex.\n4 portas de correr em Reflecta Bronze (efeito espelhado bronze) com perfil slim Bronze.\nTrilho superior embutido (sistema slim) — abertura suave com amortecimento.\nRecorte para TV fixada na 3ª porta (alinhada à cama) com cabeamento embutido.\n\nINTERIOR DO ARMÁRIO (em MDF Branco TX):\nCabideiros em diferentes alturas (1,05m e 1,95m) em tubo oblongo cromado para roupas curtas e longas.\nColuna esquerda — gaveteiro com 4 gavetas + sapateira inferior com 3 prateleiras.\nColunas centrais — cabideiros + gaveteiro com 5 gavetas (acessórios e roupas dobradas).\nColuna direita — gaveteiro com 3 gavetas + sapateira inferior + prateleiras superiores.\nPrateleiras superiores corridas para caixas e malas."},
      "Banheiro Suíte":{newName:"Banheiro Suíte",newDesc:"GABINETE INFERIOR SUSPENSO:\nGabinete suspenso 800x700x500 em MDF Gianduia.\n1 Basculante e 1 Gaveta com corrediças invisíveis e puxador cava.\n\nESPELHEIRA SUSPENSA:\nArmário espelheira 800x900x150 em MDF Gianduia com 3 portas espelhadas.\nPrateleiras internas em MDF branco tx para armazenagem de itens de higiene.\nDobradiças em inox com amortecimento.\nFita de LED embutida na borda inferior (iluminação direta sobre a bancada).\no led se extente embaixo da prateleira tb.\n\nPRATELEIRA DE APOIO:\nPrateleira lateral 600x400x150 em MDF Branco Tx com fixação oculta por grapas para apoio de plantas e decoração.\n\nFundo em MDF Branco TX.\nPuxador cava na madeira."},
      "Banheiro Social":{newName:"Banheiro Social",newDesc:"GABINETE INFERIOR SUSPENSO:\nGabinete suspenso 800x700x500 em MDF Gianduia.\n1 Basculante e 1 Gaveta com corrediças invisíveis e puxador cava.\n\nESPELHEIRA SUSPENSA:\nArmário espelheira 800x900x150 em MDF Gianduia com 3 portas espelhadas.\nPrateleiras internas em MDF branco tx para armazenagem de itens de higiene.\nDobradiças em inox com amortecimento.\nFita de LED embutida na borda inferior (iluminação direta sobre a bancada).\no led se extente embaixo da prateleira tb.\n\nPRATELEIRA DE APOIO:\nPrateleira lateral 600x400x150 em MDF Branco Tx com fixação oculta por grapas para apoio de plantas e decoração.\n\nFundo em MDF Branco TX.\nPuxador cava na madeira."},
    };
    let atualizados=0;
    const novosAmbs=orcExistente.ambientes.map(a=>{
      const c=correcoes[a.nome];
      if(c){atualizados++;return{...a,nome:c.newName,desc:c.newDesc};}
      return a;
    });
    updOrc(orcExistente.id,o=>({...o,ambientes:novosAmbs}));
    setOrcAtivo(orcExistente.id);
    setTab("orcamentos");
    showToast(`${atualizados} ambiente(s) atualizado(s) com as correções!`);
  };
  const importarOrcJoaoGonsalez=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("joão gonsalez")||c?.nome?.toLowerCase().includes("joao gonsalez");});
    if(jaExiste){return showToast("Orçamento João Gonsalez já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("joão gonsalez")||c.nome?.toLowerCase().includes("joao gonsalez"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[...p,{id:cliId,nome:"João Gonsalez",tel:"(19) 98868-0003",email:"",endereco:"",doc:""}]);}
    const ambs=[
      {id:uid(),nome:"Closet 1",desc:"Guarda roupa s/porta 2500x2700x550mm e 1450x2700x550mm.\nQuatro gavetas; quatro sapateiras deslizantes; prateleiras fixa; maleiro; canto curvo.\nEm MDF Gianduia Trama Duratex 18mm.\nFundo em MDF Gianduia Trama Duratex 06mm.\nCorrediças telescópica 30kg Hafele.\nTubo cabideiro oblongo cromado.\nPuxador em alumínio Roma Slim 210mm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Closet 2",desc:"Guarda roupa s/porta 2150x2700x550mm e 2500x2700x550mm.\nOito gavetas; seis sapateiras deslizantes; prateleiras fixa; maleiro; canto curvo.\nEm MDF Gianduia Trama Duratex 18mm.\nFundo em MDF Gianduia Trama Duratex 06mm.\nCorrediças telescópica 30kg Hafele.\nTubo cabideiro oblongo cromado.\nPuxador em alumínio Roma Slim 210mm.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Closet 3",desc:"Guarda roupa s/porta 2850x2700x550mm e 2500x2700x550mm e 1900x2700x550mm.\nOito gavetas; seis sapateiras deslizantes; prateleiras fixa; maleiro; canto curvo.\nEm MDF Gianduia Trama Duratex 18mm.\nFundo em MDF Gianduia Trama Duratex 06mm.\nCorrediças telescópica 30kg Hafele.\nTubo cabideiro oblongo cromado.\nPuxador em alumínio Roma Slim 210mm.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:"23/05/2026",status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
    setOrcamentos(p=>[...p,o]);
    setOrcAtivo(oid);
    setTab("orcamentos");
    showToast(`${num} criado — ${ambs.length} ambientes!`);
  ...
  const importarOrcNovam=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("novam");});
    if(jaExiste){return showToast("Orçamento Novam (Ottia) já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("novam"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[{id:cliId,nome:"Novam (Ottia Studio) — renomear",tel:"",email:"",endereco:"",doc:""},...p]);}
    const ambs=[
      {id:uid(),nome:"Hall — Nicho de Entrada",desc:"Nicho embutido na parede — Hall de entrada.\nDimensões externas: 520x1550mm (LxA) • profundidade 120mm.\nÁrea útil interna: 480x1510mm.\nMDF em cor a definir com cliente.\n\nCOMPOSIÇÃO INTERNA (de cima pra baixo):\n— Nicho superior decorativo 410mm de altura com cruz em MDF aplicada 300mm com braços 130+220+130mm (peça religiosa).\n— Cava usinada superior para fita de LED (branco quente).\n— Prateleira intermediária em MDF apoiada em berço a 260mm da base do nicho superior.\n— 04 ganchos metálicos 40x40mm dispostos em altura escalonada (2 níveis de 390mm cada) para pendurar bolsas.\n— Nicho inferior aberto 500x760mm para bolsas e mochilas.\n— Base de apoio 650mm de altura útil.\n\nACABAMENTOS:\n— Estrutura completamente em MDF (cor a combinar).\n— Prever cava para fita de LED superior e inferior (perfil embutido).\n— Nicho embutido totalmente na parede — prever recorte de alvenaria de aprox. 530x1560mm.\n\nRef.: Prancha 16 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha — Armários Superiores e Gabinete Inferior",desc:"Cozinha planejada — parede principal (linha da geladeira ao lava-secas).\nDimensões: 3570mm largura total x 2480mm altura (piso-teto) x profundidade 350mm (aéreo) / 600mm (gabinete inferior).\nMDF aéreos centrais: Arauco Cacao.\nMDF nicho geladeira e tamponamento: Arauco Carvalho.\nInterior: MDF Branco TX.\n\nARMÁRIOS AÉREOS (altura módulo 470mm):\n— Basculante lateral esquerdo sobre geladeira: 820x450mm (com pistão a gás).\n— 05 portas de giro em sequência: 550mm cada (5 x 550 = 2750mm).\n— Nichos decorativos abertos abaixo dos aéreos centrais: 500x430mm (para microondas) + 720+760mm (nichos de decoração).\n— Puxador cava 45° esculpido nas frentes.\n— Prateleira interna em cada módulo.\n\nNICHO PISO-TETO PARA GELADEIRA:\n— 820mm largura x 2000mm altura x 650mm profundidade.\n— Aberto (sem porta), fundo em MDF Arauco Carvalho.\n— Compatível com geladeira 700x750x1900mm.\n\nGABINETE INFERIOR (altura módulo 920mm, sobre sapata 150mm):\nSequência de portas/gavetas (larguras da esquerda pra direita): 820 + 730 + 500 + 820 + 680 + 670mm.\n— Nicho lava-louças (compartimento aberto de 500mm — recorte para máquina 450x610x850mm).\n— Nicho forno elétrico com gavetão inferior (820mm — recorte 420x665mm).\n— Nicho lava-seca (680mm — recorte para 600x700x850mm).\n— Puxador cava 45° nas portas e gavetas.\n\nBancada em granito (fornecimento e execução por marmoraria contratada à parte).\nEletrodomésticos (geladeira, microondas, cooktop, depurador, forno, lava-louças, lava-seca) — NÃO inclusos.\nFita LED sob armários aéreos (perfil embutido com cava usinada na base do armário).\n\nRef.: Prancha 17 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Cozinha — Despensa DML e Fruteira",desc:"Despensa e fruteira — parede lateral da cozinha (próximo à área técnica).\nDimensões: 1540mm largura x 2480mm altura x 500mm profundidade.\nMDF portas de correr: Arauco Cacao.\nMDF tamponamento aéreo e frentes de gavetas: Arauco Carvalho.\nInterior: MDF Branco / Cacao conforme detalhe.\n\nDESPENSA DML (piso-teto com portas de correr):\n— Módulo 1380x2400mm com 02 PORTAS DE CORRER em Arauco Cacao 690x2400mm cada.\n— Trilho superior slim embutido com amortecimento.\n— Interior (aberto ao correr): \n  · Prateleira superior 660x400mm.\n  · Nicho central 660x1590mm para depósito de material de limpeza (vassoura, aspirador, tábua de passar) — altura livre 1500mm.\n  · 05 prateleiras laterais direitas 250x400mm cada (sequência de alturas 400+400+400+400mm).\n\nMÓVEL BAIXO ADJACENTE (com fruteira):\n— Módulo 1920mm largura x 920mm altura x 500mm profundidade.\n— 02 portas de giro laterais 660x840mm com nicho interno DML.\n— 04 gavetas centrais em coluna 460x190mm cada (frentes passantes em Arauco Carvalho).\n— Fruteira deslizante 560x780mm com aramados em ALUMÍNIO CROMADO (5 níveis: 400+400+400+400+360mm).\n\nTAMPO SUPERIOR EM MDF ARAUCO CARVALHO:\n— Prateleira decorativa 1340x400mm com fixação oculta acima da bancada.\n— Tamponamento de canto 160mm unindo despensa ao módulo baixo (aparente na vista externa).\n\nInterior em MDF Branco TX.\nCorrediças telescópicas com amortecimento nas gavetas.\n\nRef.: Prancha 18 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala de Estar — Canto Alemão",desc:"Canto alemão em L — sala de estar (baú de armazenagem + sapateira integrada).\nDimensões planta: 1970mm x 1700mm (formato em L) x 450mm profundidade da base.\nAltura útil de assento: 700mm.\nMDF estrutura externa: Arauco Cacao.\nMDF interior dos baús: Arauco Branco.\n\nMODULAÇÃO EM L (lateral maior 1970mm + lateral menor 1700mm):\nLateral maior (1970mm):\n— Peitoril reto 1200mm com TAMPO BASCULANTE de 350mm de largura x 700mm profundidade (com pistão a gás) sobre baú de armazenagem interno.\n— Peitoril reto 770mm com 02 portas de giro frontais 320x320mm + sapateira em nicho aberto sob banco (2 níveis 140+140mm).\nLateral menor (1700mm):\n— Baú de armazenagem 1250mm com tampo basculante único 770x450mm (pistão a gás).\n— Peça de canto em 90°: 450x450mm.\n\nESTRUTURA:\n— Base do assento a 450mm do piso.\n— Encosto em MDF Arauco Cacao 250mm altura.\n— Nicho para sapatos com profundidade 450mm + prateleira interna em MDF Arauco Cacao.\n— Base sobre sapata rebaixada 150mm.\n\nAlmofadas soltas em linho cinza claro (fornecimento de terceiros — NÃO inclusas neste orçamento).\n\nRef.: Prancha 19 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala de Estar — Rack TV suspenso",desc:"Rack suspenso — sala de estar (parede da TV).\nDimensões: 1340mm largura x 650mm altura x 350mm profundidade.\nMDF principal (rack ripado): Arauco Carvalho.\nMDF prateleira do topo: Arauco Cacao.\n\nRACK SUSPENSO (parte principal 1020x650mm):\n— 02 portas de giro com RIPADO VAZADO (ripas verticais em MDF Arauco Carvalho — permite ventilação e passagem de sinal IR para som ambiente).\n— Cada porta 400x400mm de altura útil.\n— Interior em MDF Branco (para receiver, decoders, roteadores etc).\n— Estrutura em MDF Arauco Carvalho 350mm de profundidade.\n\nNICHO ADEGA CLIMATIZADA (lateral esquerda — 320x650mm):\n— Espaço aberto 320mm largo x 630mm de altura útil (para adega 250x510x610mm — não inclusa).\n— Ventilação inferior 100mm (frestas).\n— Prateleira em MDF Arauco Cacao 320x150mm no topo.\n\nPRATELEIRA DECORATIVA SUPERIOR:\n— Prateleira aparente 1340x350mm em MDF Arauco Cacao (visível pela vista lateral).\n— Recuo superior 150mm e inferior 100mm.\n\nFIXAÇÃO OCULTA (sem apoio no piso — efeito flutuante).\nPrever passa-fio Ø60mm no fundo para cabeamento TV.\n\nRef.: Prancha 20 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Varanda Gourmet — Armários",desc:"Marcenaria da varanda gourmet — armários aéreos + gabinete inferior.\nDimensões conjunto: 1960mm largura.\nAltura armário aéreo: 870mm • gabinete inferior: 650mm (altura módulo) + 500mm bancada existente.\nMDF externo aéreo: Arauco Cacao.\nMDF gabinete inferior: Arauco Carvalho.\nInterior: MDF Branco / Carvalho.\n\nARMÁRIO AÉREO PISO-TETO:\n— Dimensões: 1950mm largura x 870mm altura x 450mm profundidade.\n— 04 PORTAS DE GIRO simétricas de 390mm cada (2 x 780mm):\n  · 02 portas em MDF LISO Arauco Cacao (lado esquerdo).\n  · 02 portas em VIDRO INCOLOR com esquadria em Arauco Cacao (lado direito).\n— 02 prateleiras internas por lado em MDF Arauco Carvalho (390+390mm de altura útil).\n— Cava para fita de LED por baixo de cada prateleira (branco quente).\n— Nicho central aberto 770mm sobre depurador.\n— Puxador cava embutido no perfil superior.\n\nGABINETE INFERIOR (sob bancada existente):\n— Dimensões: 1960mm largura x 650mm altura x 500mm profundidade.\n— Peça esquerda: 02 portas de giro 840mm (2x 420mm) em MDF Arauco Carvalho.\n— Nicho central 450mm para adega climatizada (existente).\n— Peça direita: 02 portas de giro 670mm (2x 335mm) em MDF Arauco Carvalho.\n— Altura porta 710mm. Base 100mm.\n— Interior em MDF Branco.\n\nBANCADA em granito EXISTENTE — MANTER (não inclusa).\nChurrasqueira, cooktop, depurador, adega, cachepot: NÃO inclusos (só marcenaria).\n\nRef.: Prancha 21 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Dormitório 1 (Infantil) — Prateleira Suspensa com LED",desc:"Prateleira suspensa decorativa — Dormitório 1 (infantil).\nDimensões: 1500mm largura x 400mm profundidade x 40mm espessura.\nAltura de fixação: 1400mm do piso.\nMDF em cor a definir (Arauco Cacao ou Carvalho — conforme desejo do cliente).\n\nExecução:\n— Fixação oculta por grapas na parede.\n— Cava usinada na face inferior com perfil de LED embutido (fita LED branco quente).\n— Prever ponto elétrico dentro da prateleira para alimentação da fita.\n\nRef.: Prancha 14 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Dormitório 1 (Infantil) — Guarda-roupa 2 portas de correr",desc:"Guarda-roupa 2 portas de correr — Dormitório 1 (infantil).\nDimensões: 1850mm largura x 2800mm altura x 560mm profundidade.\nMDF tamponamento e estrutura: Arauco Cacao.\nMDF portas de correr: Arauco Beige.\nInterior: MDF Branco TX.\n\nVISTA EXTERNA:\n— 02 portas de correr 850x2680mm cada.\n— Tamponamento superior 50mm.\n— Tamponamento lateral direito 150mm (perfil decorativo aparente).\n— Trilho superior slim embutido com amortecimento.\n\nINTERIOR (1660mm útil, dividido em 02 colunas de 830mm):\n\nColuna esquerda (área de roupas):\n— Maleiro superior 830x520mm.\n— Cabideiro 830x1200mm (tubo oblongo cromado).\n— 03 gavetas 830x200mm cada.\n— Sapateira inferior 830x200mm com prateleira inclinada.\n\nColuna direita (área de roupas):\n— Maleiro superior 830x520mm.\n— Cabideiro 830x1200mm (tubo oblongo cromado).\n— 03 gavetas 830x200mm cada.\n— Sapateira inferior 830x200mm.\n\nPRATELEIRA DECORATIVA LATERAL (integrada ao tamponamento direito):\n— Prateleira 660x560mm em MDF Arauco Cacao (visível pela vista lateral).\n\nCorrediças telescópicas com amortecimento nas gavetas.\nFundo em MDF Arauco Cacao (visto pela abertura).\n\nRef.: Prancha 22 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte — Guarda-roupa 3 portas de correr",desc:"Guarda-roupa 3 portas de correr — Suíte (Dormitório 2).\nDimensões: 2280mm largura x 2800mm altura x 560mm profundidade.\nMDF tamponamento e estrutura: Arauco Cacao.\nMDF portas de correr: Arauco Beige.\nInterior: MDF Branco TX.\n\nVISTA EXTERNA:\n— 03 portas de correr: 770 + 750 + 770mm (com trilho superior slim embutido, amortecimento).\n— Tamponamento superior 50mm.\n\nINTERIOR (2240mm útil, dividido em 03 colunas de 745mm — 1/3 cada):\n\nColuna esquerda (roupas dobradas):\n— Maleiro superior 745x520mm.\n— Cabideiro 745x1200mm (tubo oblongo cromado).\n— 04 gavetas passantes 745x200mm cada.\n\nColuna central (roupas dobradas + acessórios):\n— Maleiro superior 745x520mm.\n— Cabideiro 745x1200mm.\n— 04 gavetas 745x200mm.\n\nColuna direita (prateleiras + gaveteiro):\n— Maleiro superior 745x520mm.\n— 03 prateleiras internas (alturas 380+380+390mm).\n— Gaveteiro inferior 745x900mm com 04 gavetas passantes.\n\nCorrediças telescópicas com amortecimento em todas as gavetas.\nFundo em MDF Arauco Cacao.\n\nRef.: Prancha 23 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte — Cabeceira em L com Criados-mudos e Home Office",desc:"Conjunto integrado cabeceira + criados-mudos suspensos + home office (formato em L) — Suíte.\nMDF: Arauco Carvalho (todo o conjunto).\nInterior das gavetas: MDF Branco TX.\n\nCABECEIRA (parede da cama — 3370mm largura x 1100mm altura x 80mm espessura):\n— Painel horizontal em MDF Arauco Carvalho.\n— Composição da esquerda pra direita:\n  · Mesa suspensa esquerda 1050x350mm x 300mm profundidade com 02 gavetas frontais.\n  · Cabeceira central acolchoada 1650x1100mm com CAVA USINADA superior para fita de LED (branco quente).\n  · Mesa suspensa direita 600x350mm x 300mm profundidade com 01 gaveta frontal.\n— Altura das mesas suspensas: 750mm do piso.\n— 02 caixas de tomada embutidas na marcenaria (uma em cada mesa suspensa) — externa 40x36mm, interna 10x8mm.\n\nHOME OFFICE (parede lateral perpendicular — formando L):\n— Mesa suspensa 2380mm largura x 500mm profundidade x 80mm espessura em MDF Arauco Carvalho.\n— Altura tampo: 750mm do piso.\n— Estrutura interna com nichos aparentes 400+400+780+400+400mm.\n— Passa-fios integrado ao tampo Ø60mm.\n— Cava para fita de LED por baixo (branco quente).\n\nENCONTRO EM L: prever esquadria embutida em 90° no encontro cabeceira/home office.\nCabeceira estofada em veludo/linho (cor a definir) — NÃO inclusa nesta marcenaria.\nFornecimento de tomadas e passa-fios (previsão de furação) — instalação elétrica por eletricista contratado à parte.\n\nRef.: Prancha 24 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho 1 — Espelheira Suspensa e Gabinete",desc:"Espelheira suspensa + gabinete inferior — Banho 1.\nBANCADA e CUBA existentes — MANTER (não inclusas).\nMDF: Arauco Cacao.\nInterior: MDF Branco TX.\n\nESPELHEIRA SUSPENSA (armário-espelho com 3 portas de correr):\n— Dimensões: 1400mm largura x 1050mm altura x 120mm profundidade.\n— 03 portas de correr ESPELHADAS 466x850mm cada (1/3 + 1/3 + 1/3 = 466 + 466 + 466mm).\n— Trilho superior e inferior slim embutido.\n— Interior útil 1360mm dividido em 03 compartimentos (440 + 430 + 440mm).\n— 02 prateleiras internas em MDF Branco TX em cada vão (alturas 405 + 405mm).\n— Nicho aberto inferior 1360x180mm (para sabonete líquido, escova de dentes, etc).\n— Cava para fita de LED entre espelheira e bancada.\n\nGABINETE INFERIOR (suspenso, fixado à parede):\n— Dimensões: 520mm largura x 600mm altura x 660mm profundidade.\n— 02 PORTAS BASCULANTES horizontais 520x300mm cada (com pistão a gás).\n— Interior em MDF Branco TX.\n— Cava superior para fita de LED (efeito flutuante).\n\nRef.: Prancha 25 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Banho 2 — Espelheira Suspensa e Gabinete",desc:"Espelheira suspensa + gabinete inferior — Banho 2.\nBANCADA e CUBA existentes — MANTER (não inclusas).\nMDF: Arauco Carvalho (diferente do Banho 1).\nInterior: MDF Branco TX.\n\nESPELHEIRA SUSPENSA (armário-espelho com 3 portas de correr):\n— Dimensões: 1450mm largura x 1050mm altura x 120mm profundidade.\n— 03 portas de correr ESPELHADAS 483x850mm cada (1/3 + 1/3 + 1/3 = 483 + 483 + 483mm).\n— Trilho superior e inferior slim embutido.\n— Interior útil 1410mm dividido em 03 compartimentos (460 + 450 + 460mm).\n— 02 prateleiras internas em MDF Branco TX em cada vão (alturas 405 + 405mm).\n— Nicho aberto inferior 1410x180mm (para sabonete líquido, escova de dentes, etc).\n— Cava para fita de LED entre espelheira e bancada.\n\nGABINETE INFERIOR (suspenso, fixado à parede):\n— Dimensões: 600mm largura x 600mm altura x 660mm profundidade.\n— 02 PORTAS BASCULANTES horizontais 600x300mm cada (com pistão a gás).\n— Interior em MDF Branco TX.\n— Cava superior para fita de LED.\n\nRef.: Prancha 26 — Ottia Studio.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
    ...
    setOrcAtivo(oid);
    setTab("orcamentos");
    showToast(`${num} criado — ${ambs.length} ambientes (Projeto Ottia)!`);
  };
  ...
  const importarOrcDaniFabio=()=>{
    ...
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("dani") && c?.nome?.toLowerCase().includes("fábio"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[{id:cliId,nome:"Dani e Fábio",tel:"",email:"",endereco:"Av. Luiz Pereira Leite, 1151 — Condomínio Village Água Branca, Piracicaba/SP",doc:""},...p]);}
    const ambs=[
      {id:uid(),nome:"Hall — Painel Ripado + Gaveteiro Suspenso",desc:"PAINEL RIPADO (Item 01)\nDimensões: 1355x2340mm (LxA).\nMDF Freijó Puro Duratex, profundidade 40mm.\nRipas de 20x20mm com espaçamento de 15mm entre ripas.\nFixação oculta na parede.\nPrever 04 ganchos para pendurar (ver detalhe — ganchos metálicos sobrepostos à ripa).\n\nGAVETEIRO SUSPENSO (Item 02)\nDimensões: 1205x335x200mm (LxAxP).\nFixação suspensa no painel ripado (sem apoio no piso).\nFrentes externas em MDF Freijó Puro Duratex 18mm.\nInterior, divisórias e fundo em MDF Branco TX Duratex 15mm e 06mm respectivamente.\n02 gavetas (largura total ~120cm) com puxador inferior cava 45° esculpida na própria frente.\nCorrediças telescópicas com amortecimento.\n\nObservações:\n— Realocar interruptores e tomadas existentes para frente do painel.\n— Espelho existente no hall: manter; o gaveteiro fica abaixo do espelho.\n— Considerar rodapé existente.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala — Rack",desc:"RACK COMPLETO (Item 03)\nDimensões: 3455x800x450mm (LxAxP).\nEstrutura, frentes e tampo em MDF Branco TX Duratex 18mm.\nFundo em MDF Branco TX Duratex 06mm.\n\nDIVISÃO (5 módulos):\n— 02 módulos laterais esquerdos (593,8mm cada) com porta de giro.\n— 01 módulo central (500mm) tipo nicho aberto para TV / decoração com PRATELEIRA central em MDF Branco TX 18mm.\n— 02 módulos laterais direitos (593,8mm e 603,8mm) com porta de giro.\n— Total: 04 PORTAS DE GIRO em MDF Branco TX 18mm com amortecimento.\n— Puxador tipo cava 45° esculpido nas próprias frentes (ver detalhe).\n— Prateleiras internas em MDF Branco TX 18mm.\n\nINSTALAÇÃO E FIAÇÃO:\n— Altura tampo: 800mm do piso (suspenso, sem apoio).\n— Prever PASSA FIO Ø60mm em 02 pontos do nicho central da prateleira (lado esquerdo e direito), para fios de TV e equipamentos.\n— Considerar rodapé existente.\n— Realocar tomadas/interruptores existentes para trás do rack.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Sala de Jantar — Cristaleira + Prateleiras + Bancada",desc:"CRISTALEIRA (Item 04)\nDimensões: 1095x2130x470mm (LxAxP).\nEstrutura e fechamento em MDF Branco TX Duratex 18mm.\n02 portas de giro em vidro REFLECTA BRONZE com perfil metálico bronze e amortecimento.\n02 puxadores tipo aba slim sobrepor bronze 12mm (referência).\nPrateleiras internas em vidro com PERFIL DE LED EMBUTIDO por baixo de cada prateleira (fita LED branco quente).\nAltura igualada à viga existente.\nCristaleira COM FUNDO em MDF Branco TX 06mm.\n\nPRATELEIRAS SUSPENSAS (Item 05)\nMDF Branco TX Duratex 18mm, profundidade 300mm.\nFixação oculta na parede.\nPERFIL DE LED EMBUTIDO por baixo de cada prateleira (fita branco quente).\n\nBANCADA (Item 06)\nDimensões: ~950mm altura (mesa).\nMDF Freijó Puro Duratex 18mm (tampo e estrutura).\nFechamento lateral em chanfro 45° (encontro perfeito sem testeira aparente).\n\nObservações gerais do ambiente:\n— Fitas LED em cor branco quente.\n— Botão interruptor paralelo LED.\n— Realocar interruptores e tomadas existentes conforme indicado em projeto.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Suíte — Cabeceira + Painel com Porta + Prateleira",desc:"CABECEIRA (Item 07)\nDimensões: 2620x1200mm (LxA), profundidade 40mm.\nMDF Branco TX Duratex.\nPainel com FRISOS USINADOS de 5mm a cada 65cm (4 frisos verticais).\nLED EMBUTIDO POR CIMA da cabeceira (fita LED branco quente).\nIncluir 02 botões interruptores paralelos na cor branca embutidos na própria cabeceira (cada lado da cama).\nConsiderar rodapé existente.\n\nPAINEL COM PORTA DE CORRER (Item 08)\nDimensões painel: 2100x2740mm (LxA), profundidade 80mm.\nMDF Branco TX Duratex.\nPorta de correr 700x2100mm EMBUTIDA NO PAINEL (correndo por trás do painel) com estrutura interna reforçada.\nPuxador cava esculpida no painel (ver detalhe).\nPrever vão de 10mm entre porta e parede e entre painel e porta.\nAR CONDICIONADO realocado para frente do painel — prever aberturas necessárias no painel para tubulações.\nRealocar tomada existente para cima do painel.\n\nPRATELEIRA SUSPENSA (Item 09)\nDimensões: 1950x100x250mm (LxAxP).\nMDF Branco TX Duratex 18mm.\nFixação oculta acima do painel/porta.\nPASSA FIO previsto no painel para alimentação da prateleira (LED opcional).",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Escritório — Armário Superior + Prateleira + Gaveteiro Fixo + Bancada",desc:"ARMÁRIO SUPERIOR (Item 10)\nDimensões: 2850x815x450mm (LxAxP).\nMDF Branco TX Duratex 18mm.\n06 portas de giro 475x815mm com amortecimento.\nPuxador tipo cava 45° esculpida nas frentes (ver detalhe).\nPrateleira interna em MDF Branco TX 18mm.\n\nPRATELEIRA SUSPENSA (Item 11)\nDimensões: ~2850x350mm (LxP), espessura 18mm.\nMDF Branco TX Duratex.\nFixação oculta entre o armário superior e a bancada.\n\nGAVETEIRO FIXO (Item 12)\nDimensões: 450x620x400mm (LxAxP).\nMDF Branco TX Duratex.\n04 gavetas com frente passante e puxador tipo cava 45° esculpida (ver detalhe).\nCorrediças telescópicas com amortecimento (capacidade compatível com a largura).\nInterior em MDF Branco TX 15mm, fundo 06mm.\n\nBANCADA DUPLA (Item 13)\nDimensões tampo: 1900x600x?? (LxPxesp.) — profundidade 600mm.\nMDF Branco TX Duratex.\nPORTA FIOS: 02 TAMPOS BASCULANTES com recuo de 15mm para puxador cava e passagem dos fios.\nPASSA FIO Ø60mm — 02 furos para baixo da bancada (um por estação).\nFIXAÇÃO REFORÇADA na parede para suportar peso de monitores/equipamentos.\nFechamento inferior até a parede (ambos os lados).\n\nObservações gerais do ambiente:\n— Fitas LED cor branco quente.\n— Realocar interruptores e tomadas existentes conforme indicado.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
  ...
  const importarOrcCamilaCasarin=()=>{
    const jaExiste=orcamentos.find(o=>{const c=clientes.find(x=>x.id===o.clienteId);return c?.nome?.toLowerCase().includes("camila casarin");});
    if(jaExiste){return showToast("Orçamento Camila Casarin já existe!","red");}
    let cliId=clientes.find(c=>c.nome?.toLowerCase().includes("camila casarin"))?.id;
    if(!cliId){cliId=uid();setClientes(p=>[{id:cliId,nome:"Camila Casarin",tel:"",email:"",endereco:"",doc:""},...p]);}
    const ambs=[
      {id:uid(),nome:"Quarto — Guarda-roupa",desc:"Guarda-roupa de correr 1900x2500x650mm.\nEstrutura externa em MDF Cromio Berneck 18mm.\n\nPORTAS DE CORRER (2 unidades):\n2 portas 845x2350mm em MDF Carvalho Batur Duratex 18mm.\nSistema de trilho superior embutido slim com amortecimento.\nRecuos de 15mm superior, 5mm laterais e 7mm inferior para fechamento perfeito.\n\nDIVISÃO INTERNA (2 colunas de 840mm cada):\n\nColuna esquerda — área de roupas:\nMaleiro superior 840x400mm em MDF Branco TX 15mm.\nCabideiro superior 950mm de altura em tubo oblongo cromado.\nCabideiro inferior 950mm de altura em tubo oblongo cromado.\n\nColuna direita — prateleiras + gavetas:\nMaleiro superior 840x400mm em MDF Branco TX 15mm.\n2 níveis de prateleiras (configuração 2x2, 410mm largura x 460mm altura cada vão).\n4 gavetas inferiores 820x220mm com frente passante (sem puxador aparente).\n\nInterior e prateleiras em MDF Branco TX 15mm.\nFundo em MDF Branco TX 06mm.\nCorrediças telescópicas 30kg Hafele com amortecimento.\nTubo cabideiro oblongo cromado.",insumos:[],vi:0,valorTotal:0},
      {id:uid(),nome:"Quarto — Cômoda",desc:"Cômoda 1240x950x500mm.\nEstrutura externa e tampo em MDF Carvalho Batur Duratex 18mm.\n\n8 gavetas distribuídas em 4 linhas x 2 colunas:\nFrentes 610x190mm cada em MDF Cromio Berneck 18mm.\nFrente passante (sem puxador aparente — abertura por cava ou tip-on).\n\nInterior das gavetas e divisórias em MDF Branco TX 15mm.\nFundo em MDF Branco TX 06mm.\nCorrediças telescópicas 30kg Hafele com amortecimento.\nBase com recuo de 60mm para iluminação indireta opcional.",insumos:[],vi:0,valorTotal:0},
    ];
    const oid=uid();
    const num=`ORC-${String(orcamentos.length+1).padStart(4,"0")}`;
    const o={id:oid,num,clienteId:cliId,data:hoje(),status:"rascunho",ambientes:ambs,garantia:empresa.garantia||GARANTIA,garantiaE:false,pagamento:empresa.pagamento||PAGAMENTO,pagamentoE:false,markup:MARKUP,desconto:0,vendedorId:"",percNF:0,especificacoes:empresa.especificacoes||ESPECIFICACOES,especificacoesE:false,validade:mesVigente(),prazoEntrega:empresa.prazoExecucao||""};
  ...
  const PgPed=()=>{
    ...
    const META=(empresa.metas||{})[pedMes]||200000;
    // p.data está no formato DD/MM/YYYY — converte para YYYY-MM para comparar
    const pMes=d=>{if(!d)return"";if(/^\d{4}-/.test(d))return d.slice(0,7);const pts=d.split("/");return pts.length===3?`${pts[2]}-${pts[1]}`:""};
    ...
    return(<div style={{animation:"fadeIn .3s"}}><SH title="Pedidos" sub={`${pedidosMes.length} em ${nomeMesPed}`}/>
      <Card style={{padding:18,marginBottom:16,background:"linear-gradient(135deg,#0f172a,#1e293b)",border:"none"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <span style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"capitalize",letterSpacing:"0.5px"}}>{nomeMesPed}</span>
          <button onClick={()=>navPedMes(1)} style={{background:"rgba(255,255,255,.1)",border:"none",color:"#94a3b8",borderRadius:6,width:26,height:26,cursor:"pointer",fontSize:18,lineHeight:"26px",textAlign:"center"}}>›</button>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
  ...
  const PgDRE=()=>{
    ...
    // Periodo dinamico: se mes selecionado, filtra TODA a DRE pelo mes; senao, ano inteiro
    const prefx=dreMes||anoStr;
    const MESES_FULL=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
    const periodoLabel=dreMes?`${MESES_FULL[+dreMes.slice(5,7)-1]}/${dreMes.slice(0,4)}`:anoStr;
    // Filtro de pedidos: por mes (DD/MM/YYYY) quando dreMes setado, senao por ano
    const matchPed=p=>{
      if(!p.data)return false;
      const[d,mo,a]=p.data.split("/");
      if(dreMes){const[aaaa,mm]=dreMes.split("-");return a===aaaa&&mo===mm;}
      return a===anoStr;
    };
    const pedAno=pedidos.filter(p=>matchPed(p)&&p.status!=="cancelado");
    const finAno=financeiro.filter(f=>f.parcelas?.some(p=>p.venc?.startsWith(prefx)));
    ...
    const recFinMan=financeiro.filter(f=>f.tipo==="receber"&&!f.pedidoId).reduce((s,f)=>s+f.parcelas.filter(p=>p.venc?.startsWith(prefx)).reduce((ss,p)=>ss+p.valor,0),0);
    ...
    // Normaliza string: sem acentos, lowercase, trim - usado para mesclar categorias duplicadas
    ...
    const ccFin=financeiro.filter(f=>f.tipo==="pagar"&&isComFin(f)&&!isMaterialFin(f)&&!(f.marcId&&f.pedidoId)).reduce((s,f)=>s+valorPagoNoPeriodo(f,prefx),0);
    ...
    const despesasFin=financeiro.filter(filtroDespesas).reduce((s,f)=>s+valorPagoNoPeriodo(f,prefx),0);
    const despesasFinPagas=despesasFin;
    const lb=rec-cmTotal;const ll=lb-ccTotal-despesasFin;const mg=rec>0?((ll/rec)*100).toFixed(1):0;
    const rows=[
      {l:"(+) Receita Bruta",v:rec,c:"gn",b:true},
      {l:"(−) Custo de Materiais",v:-cmTotal,c:"rd"},
      {l:"    ↳ dos Pedidos",v:-cm,c:"rd",sub:true},
      {l:"    ↳ Compras (Financeiro)",v:-cmFin,c:"rd",sub:true},
      {l:"= Lucro Bruto",v:lb,c:lb>=0?"gn":"rd",b:true,line:true},
      {l:"(−) Comissões",v:-ccTotal,c:"am"},
      {l:"    ↳ dos Pedidos",v:-cc,c:"am",sub:true},
      {l:"    ↳ Lançamentos (Financeiro)",v:-ccFin,c:"am",sub:true},
      {l:"(−) Despesas Operacionais",v:-despesasFin,c:"rd"},
      {l:"    ↳ Já pagas",v:-despesasFinPagas,c:"rd",sub:true},
      {l:"= Resultado Líquido",v:ll,c:ll>=0?"gn":"rd",b:true,line:true}
    ];
    const chartData=[{name:"Receita",value:rec},{name:"Materiais",value:cmTotal},{name:"Comissões",value:ccTotal},{name:"Despesas",value:despesasFin},{name:"Lucro Líq.",value:Math.max(0,ll)}];
    ...
    const mensalData=MESES.map((m,i)=>{
      ...
      const pedM=pedidos.filter(p=>{const[d,mo,a]=p.data?.split("/")||[];return a===anoStr&&mo===mm&&p.status!=="cancelado";});
      const recMPed=pedM.reduce((s,p)=>s+p.vt,0);
    ...
    financeiro.filter(f=>f.tipo==="pagar").forEach(f=>{
      ...
      if(!rawCat){
        if(isMaterialFin(f))rawCat="Materiais (sem categoria)";
        else if(isComFin(f))rawCat="Comissões (sem categoria)";
        else rawCat="Sem categoria";
      }
      const key=norm(rawCat);
    ...
    return(<div style={{animation:"fadeIn .3s"}}><SH title={`DRE — ${periodoLabel}`} sub={dreMes?"Cartão = valor TOTAL no mês da compra • Demais = parcelas pagas":"Selecione um mês para isolar o resultado mensal"} right={<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><select value={dreAno} onChange={e=>setDreAno(+e.target.value)} style={{padding:"7px 12px",borderRadius:10,border:"1.5px solid var(--bd)",background:"var(--sf)",color:"var(--tx)",fontSize:12,fontWeight:700,outline:"none"}}>{[...new Set([dreAno,new Date().getFullYear(),new Date().getFullYear()-1])].sort().reverse().map(a=><option key={a} value={a}>{a}</option>)}</select><Btn v="ghost" small onClick={print}><I.Printer/> Fechar Ano</Btn><input type="month" value={dreMes} onChange={e=>setDreMes(e.target.value)} style={{padding:"6px 10px",borderRadius:10,border:"1.5px solid var(--bd)",background:"var(--sf)",color:"var(--tx)",fontSize:12,fontWeight:700,outline:"none"}}/>{dreMes&&<Btn v="secondary" small onClick={printMes}><I.Printer/> Fechar Mês</Btn>}{dreMes&&<button onClick={()=>setDreMes("")} style={{background:"none",border:"none",color:"var(--tx3)",cursor:"pointer",fontSize:12}}>✕</button>}</div>}/>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:820}}>
            <thead><tr style={{background:"var(--bg)"}}>
              {["Mês","Receita","(-) Materiais","(-) Comissões","(-) Despesas","= Resultado"].map((h,hi)=><th key={h} style={{padding:"8px 12px",fontSize:9,fontWeight:800,textTransform:"uppercase",color:"var(--tx3)",borderBottom:"1.5px solid var(--bd)",textAlign:hi===0?"left":"right"}}>{h}</th>)}
            </tr></thead>
            <tbody>
              {mensalData.map((m,i)=>{const temDados=m.receita+m.materiais+m.comissoes+m.despesas>0;return(<tr key={i} style={{background:i%2===0?"transparent":"var(--bg)"}}>
                ...
                <td style={{padding:"8px 12px",textAlign:"right",fontWeight:700,color:"var(--rd)",fontSize:12}}>{m.materiais>0?R$(m.materiais):"—"}</td>
                <td style={{padding:"8px 12px",textAlign:"right",fontWeight:700,color:"var(--am)",fontSize:12}}>{m.comissoes>0?R$(m.comissoes):"—"}</td>
                <td style={{padding:"8px 12px",textAlign:"right",fontWeight:700,color:"var(--rd)",fontSize:12}}>{m.despesas>0?R$(m.despesas):"—"}</td>
                <td style={{padding:"8px 12px",textAlign:"right",fontWeight:800,color:m.resultado>=0?"var(--gn)":"var(--rd)",fontSize:13}}>{temDados?R$(m.resultado):"—"}</td>
              </tr>);})}
              <tr style={{background:"var(--prib)",borderTop:"2px solid var(--bd)"}}>
                ...
                <td style={{padding:"10px 12px",textAlign:"right",fontWeight:800,color:"var(--rd)",fontSize:13}}>{R$(cmTotal)}</td>
                <td style={{padding:"10px 12px",textAlign:"right",fontWeight:800,color:"var(--am)",fontSize:13}}>{R$(ccTotal)}</td>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
        ...
        {dreCatDetalhe&&(()=>{
          ...
          const editarCat=(l)=>{
            const sugestoes=catsExistentes.length?`\n\nCategorias já usadas:\n${catsExistentes.join(", ")}`:"";
            const nova=window.prompt(`Editar categoria do lançamento:\n"${l.desc}"\nFornecedor: ${l.fornecedor}${sugestoes}\n\nDigite a categoria correta:`,l.catAtual);
            if(nova===null)return;
            const trim=nova.trim();
            if(!trim||trim===l.catAtual){showToast("Nada mudou","red");return;}
            setFinanceiro(prev=>prev.map(f=>f.id===l.fid?{...f,categoria:trim}:f));
            showToast(`✅ "${l.desc}" recategorizado para "${trim}"`);
            setDreCatDetalhe(null);
          };
          return(
            <Modal onClose={()=>setDreCatDetalhe(null)} wide>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap",marginBottom:14}}>
                ...
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:9,fontWeight:800,color:"var(--tx3)",textTransform:"uppercase"}}>TOTAL PAGO NO PERÍODO</div>
                  <div style={{fontSize:22,fontWeight:800,color:"var(--gn)"}}>{R$(c.value)}</div>
                  <div style={{fontSize:10,color:"var(--tx3)",fontWeight:700,marginTop:4}}>Apenas parcelas efetivamente pagas</div>
                </div>
              </div>
        ...
        {catData.length>0&&<Card><CardHead title={`Todos os Gastos por Categoria — ${periodoLabel}`} right={<span style={{fontSize:11,color:"var(--rd)",fontWeight:800}}>Total: {R$(totalGastosCat)}</span>}/><div style={{padding:14,maxHeight:480,overflowY:"auto"}}>{catData.map((c,i)=>{const total=catData.reduce((s,x)=>s+x.value,0);const perc=total>0?(c.value/total*100):0;return(<div key={i} onClick={()=>setDreCatDetalhe(c)} className="hr" style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8,alignItems:"center",padding:"10px 8px",borderBottom:"1px solid var(--bd)",fontSize:12,cursor:"pointer",borderRadius:6}}><div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><span style={{fontWeight:700,color:"var(--tx)"}}>{c.name}</span><span style={{fontSize:10,color:"var(--tx3)",fontWeight:700,marginLeft:8}}>{perc.toFixed(1)}%</span></div><div style={{background:"var(--bg)",height:4,borderRadius:2,overflow:"hidden"}}><div style={{width:`${perc}%`,height:"100%",background:"var(--rd)",borderRadius:2}}/></div><div style={{fontSize:10,color:"var(--tx3)",marginTop:3,fontWeight:600}}>{c.lancs.length} lançamento(s) • {c.lancs.reduce((s,l)=>s+l.parcelasNoPeriodo.length,0)} parcela(s) paga(s) no período</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:800,color:"var(--rd)",fontSize:13}}>{R$(c.value)}</div><div style={{fontSize:9,color:"var(--tx3)",fontWeight:700,marginTop:2}}>VER LANÇAMENTOS →</div></div></div>);})}</div></Card>}
      </div>
    </div>);};
